...
- [BUGFIX] `scheduler`: restarting a BunkerWeb container left it serving traffic with every timer-driven plugin dead. The instance comes back and answers normally, but it keeps its startup loading state until something pushes it a configuration — and while it holds that state both Lua timer loops stop after restoring known bans, so bad-behavior counting and the bans that follow from it, the metrics buffer flush and the sessions cleanup all stop silently, with nothing in the logs to say so. The scheduler only re-pushed to an instance it had caught as `down`, which a restart of ten or twenty seconds slips between two thirty-second healthchecks. The healthcheck now asks each instance for its own state through the new `GET /instances/{hostname}/health` and re-pushes when a reachable instance still reports itself as loading, once per episode rather than on every pass. Instances reachable throughout a restart are the ones this affected; a longer outage was already handled.
