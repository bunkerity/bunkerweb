...
- [BUGFIX] `autoconf`: on Kubernetes, a BunkerWeb pod that is running but not yet Ready is discovered and receives its configuration, since the readiness probe waits for that configuration; a pod being deleted is skipped.
- [BUGFIX] `letsencrypt`: with `LETS_ENCRYPT_PASSTHROUGH=yes`, a GET or HEAD of a single ACME token under `/.well-known/acme-challenge/` is whitelisted and reaches the backend without any other check, including ModSecurity and the ban check; antibot no longer intercepts the validator. (Fixes #3927)
