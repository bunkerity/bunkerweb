...
- [BUGFIX] `letsencrypt`: an ACME account the certificate authority no longer accepts is replaced instead of leaving every renewal failing forever. A deactivated account was never even recognized as the cause, because only one of the two rejections the authority can send was matched, so every attempt kept using it. The account is now retired, a new one registered, and the renewal configurations moved onto it in the same run, on renewal as well as on issuance. The retired account is kept aside for 30 days rather than deleted, so its key stays available. No certificate is re-issued and the repair itself costs no request. (Fixes #3783)
- [BUGFIX] `letsencrypt`: a repair that could not fix every renewal configuration is now saved instead of being discarded, where one configuration whose certificate authority had no account left threw away the repair of all the others and had them redone on every run.
- [BUGFIX] `letsencrypt`: DNS credential items are split on any whitespace and the key is no longer taken verbatim, so a leading space or a quoted key stops silently discarding the credential and failing validation. (Refs #3783)
...
- [BUGFIX] `letsencrypt`: a certificate whose renewal configuration is missing or unreadable no longer ends the job for every other service, and in the default non-concurrent mode one service failing unexpectedly no longer skips the remaining services and the cache save.
