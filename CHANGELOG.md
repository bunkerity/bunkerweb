...
- [SECURITY] `api`: `API_ACL_BOOTSTRAP_FILE` now validates a supplied `password_hash`/`password_bcrypt` (real bcrypt, cost ≥ `10`); a weak/malformed hash is ignored instead of stored verbatim, closing a near-plaintext admin credential via a controlled ACL file.
