...
function getCrowdSecReportSnapshot(report, limit = 20) {
  ...
  return {
    ...
    remediation: candidate.remediation || candidate.action,
...
function isCrowdSecRemovalConfirmed(result) {
  return Boolean(result && result.removed === true);
}
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  const connectionFilter = byId("crowdsec-connection-filter");
  ...
  const formatObserved = (value) => {
    ...
    if (!Number.isFinite(number)) return t("crowdsec.unknown", "Unknown");
    return new Date(number * 1000).toLocaleString();
  };

  const formatTime = (value) => {
    if (value === undefined || value === null || value === "")
      return t("crowdsec.unknown", "Unknown");
  ...
  const fetchJson = async (url, options) => {
    ...
    const requestError = t(
      "crowdsec.error.request",
      "The CrowdSec request failed.",
    );
    if (response.redirected) throw new Error(requestError);
    let body;
    try {
    ...
    if (!response.ok) {
      throw new Error(body.error || requestError);
    }
    return body;
  };
  ...
  const connectionLabel = (connection) => {
    const services = Array.isArray(connection.services)
      ...
      : t("crowdsec.connection.global", "all services");
    return `${connection.instance || "?"} · ${services}`;
  };

  const applyConnectionFilter = () => {
    const query = connectionFilter.value.trim().toLocaleLowerCase();
    connectionGroup
      .querySelectorAll(".crowdsec-connection-card")
      .forEach((card) => {
        card.parentElement.classList.toggle(
          "d-none",
          Boolean(query) && !card.parentElement.dataset.search.includes(query),
        );
      });
  };
  ...
  const renderConnection = (connection, index) => {
    ...
    column.dataset.search = [
      connection.instance,
      ...(Array.isArray(connection.services) ? connection.services : []),
    ]
      .filter(Boolean)
      .join(" ")
      .toLocaleLowerCase();
    ...
    const badge = element(
      ...
      reachable
        ? t("crowdsec.connection.connected", "Connected")
        : failed
          ? t("crowdsec.connection.connection_error", "Connection error")
          : t("crowdsec.unknown", "Unknown"),
    );
    header.append(badge);
    body.append(header);
    if (connection.lapi_url) {
      body.append(
        element(
          ...
          `${t("crowdsec.connection.local_api", "Local API")}: ${connection.lapi_url}`,
        ),
      );
    }
    appendDetail(
      body,
      t("crowdsec.connection.mode", "Mode"),
      connection.mode || "-",
    );
    appendDetail(
      body,
      t("crowdsec.connection.management", "Management"),
      connection.management_configured
        ? t("crowdsec.connection.configured", "Configured")
        : t("crowdsec.connection.read_only", "Read only"),
    );
    const syncStatus = {
      current: [
        t("crowdsec.connection.sync_current", "Up to date"),
        "badge bg-success",
      ],
      stale: [
        t("crowdsec.connection.sync_stale", "Stale"),
        "badge bg-warning text-dark",
      ],
      not_synchronized: [
        t("crowdsec.connection.sync_not_synchronized", "Not synchronized yet"),
        "badge bg-warning text-dark",
      ],
    ...
    appendDetail(
      body,
      t("crowdsec.connection.sync", "Decision sync"),
      syncStatus[0],
      syncStatus[1],
    );
    if (connection.last_successful_sync) {
      appendDetail(
        body,
        t("crowdsec.connection.last_sync", "Last successful sync"),
    ...
    if (connection.appsec_last_observed) {
      ...
      appendDetail(
        body,
        t("crowdsec.connection.appsec", "Last AppSec observation"),
        `${formatObserved(appsec.at)} · ${appsec.status || appsec.action || t("crowdsec.unknown", "Unknown")}`,
      );
      if (appsec.error) {
        body.append(
    ...
    const selectedLabel = element(
      ...
      t("crowdsec.connection.selected", "Selected connection"),
    );
  ...
  const renderSectionUnavailable = (container, message) => {
    container.replaceChildren(
      element(
        ...
        t("crowdsec.investigation.unavailable", "Unavailable: {{error}}", {
          error: message,
        }),
      ),
    );
  ...
  const openRemoval = (decision, connection, trigger) => {
    ...
    byId("crowdsec-unban-selection").textContent = t(
  ...
  const removalButton = (decision, connection) => {
    ...
    const button = element(
      ...
      t("crowdsec.remove.action", "Remove"),
    );
    button.type = "button";
  ...
  const renderDecisions = (payload, connection) => {
    ...
    byId("crowdsec-decisions-count").textContent = payload.observed_at
      ? `${countText} · ${t("crowdsec.observed_at", "Observed {{time}}", {
  ...
  const loadDecisions = async () => {
    ...
    renderDecisionNotice(t("status.loading", "Loading..."));
    try {
  ...
  const investigationGroups = [
    {
      ...
      label: t(
        "crowdsec.investigation.summary_decisions",
        "CrowdSec decisions",
      ),
      found: (total) =>
        t("crowdsec.investigation.summary_active", "{{total}} active", {
          total,
        }),
      badge: t("crowdsec.current_state", "Current state"),
    ...
    },
    {
      ...
      label: t("crowdsec.investigation.summary_bans", "Local BunkerWeb bans"),
      found: (total) =>
        t("crowdsec.investigation.summary_active", "{{total}} active", {
          total,
        }),
    ...
    },
    {
      ...
      found: (total) =>
        t("crowdsec.investigation.summary_available", "{{total}} available", {
          total,
        }),
    },
    {
      ...
      label: t("crowdsec.investigation.summary_reports", "BunkerWeb reports"),
      found: (total) =>
        t("crowdsec.investigation.summary_captured", "{{total}} captured", {
          total,
        }),
      badge: t("crowdsec.captured_history", "Captured history"),
  ...
  const summaryTone = (group, section) => {
    if (!section.available)
      return [
        "is-unreadable",
        "",
        t("crowdsec.investigation.summary_unreadable", "Could not be read"),
      ];
    if (!section.total)
      return [
        "",
        "text-muted",
        t("crowdsec.investigation.summary_none", "None found"),
      ];
  ...
  const renderInvestigationSummary = (sections) => {
    ...
    investigationGroups.forEach((group) => {
      ...
      const label = element("span", "small text-muted", group.label);
      const status = element("span", `fw-semibold ${tone}`.trim(), state);
  ...
  const renderInvestigationDecisions = (section, connection) => {
    ...
    section.items.forEach((decision) => {
      const card = listCard(
        ...
        [
          [t("crowdsec.decision.type", "Type"), decision.type],
          [t("crowdsec.decision.origin", "Origin"), decision.origin],
          [t("crowdsec.decision.scenario", "Scenario"), decision.scenario],
          [t("crowdsec.decision.duration", "Duration"), decision.duration],
        ],
        t("crowdsec.current_state", "Current state"),
      );
      const button = removalButton(decision, connection);
      if (button) {
  ...
  const renderBans = (section, observedAt) => {
    ...
    section.items.forEach((ban) => {
      ...
      container.append(
        listCard(ban.ip || "-", [
          [t("crowdsec.local_ban.reason", "Reason"), ban.reason],
          [t("crowdsec.local_ban.service", "Service"), ban.service],
          [
            t("crowdsec.local_ban.expires", "Expires"),
            ban.permanent
              ? t("scope.permanent", "Permanent")
              : expiresAt !== null
  ...
  const renderAlertEvidence = (container, alert) => {
    ...
    const details = listCard(
      ...
      [
        [t("crowdsec.alert.id", "Alert ID"), alert.id],
        [t("crowdsec.decision.scenario", "Scenario"), alert.scenario],
        [t("crowdsec.alert.started", "Started"), formatTime(alert.start_at)],
        [t("crowdsec.alert.stopped", "Stopped"), formatTime(alert.stop_at)],
      ],
    );
  ...
  const renderAlerts = (section, connection) => {
    ...
    section.items.forEach((alert) => {
      ...
      const button = element(
        ...
        t("crowdsec.alert.load", "Load full evidence"),
      );
      button.type = "button";
      button.addEventListener("click", () =>
  ...
  const reportSourceLabel = (source) =>
    ...
    })[source] || t("crowdsec.report.entry", "Security report");
  ...
  const appendReportTechnical = (card, report, snapshot) => {
    ...
    const fields = [
      [
        t("crowdsec.report.identifier", "Report identifier"),
    ...
    if (snapshot) {
      fields.push(
        ...
        [t("crowdsec.report.source", "Captured source"), snapshot.source],
        [
          t("crowdsec.report.scope", "Captured service scope"),
        ...
        [
          t("crowdsec.report.metadata", "Decision metadata"),
          snapshot.metadataAvailable
            ? t("crowdsec.report.metadata_complete", "Complete")
            : t("crowdsec.report.metadata_incomplete", "Incomplete"),
        ],
      );
    }
    ...
    snapshot.decisions.forEach((decision) => {
      ...
      captured.append(
        element(
          ...
          decision.scenario || t("crowdsec.unknown", "Unknown"),
        ),
      );
      captured.append(
        fieldList(
          [
            [t("crowdsec.decision.origin", "Origin"), decision.origin],
            [t("crowdsec.decision.scope", "Scope"), decision.scope],
            [t("crowdsec.decision.target", "Target"), decision.target],
            [
            ...
            [t("crowdsec.decision.id", "Decision ID"), decision.id],
          ],
          "row g-2 mb-0 small",
        ),
      );
  ...
  const renderReports = (section) => {
    ...
    section.items.forEach((report) => {
      ...
      const fields = [
        [
          t("crowdsec.report.date", "Report time"),
          Number.isFinite(Number(report.date))
            ? formatObserved(report.date)
            : report.date,
        ],
        [t("crowdsec.report.service", "Service"), report.server_name],
        [t("crowdsec.report.reason", "Reason"), report.reason],
      ];
      if (snapshot)
        fields.push([
      ...
      const card = listCard(
        snapshot
          ...
          : t("crowdsec.report.entry", "Security report"),
        fields,
        t("crowdsec.captured_history", "Captured history"),
      );
  ...
  const loadAllowlists = async () => {
    ...
    container.replaceChildren(emptyState(t("status.loading", "Loading...")));
    try {
      const query = new URLSearchParams({
        connection: connection.id,
      ...
      items.forEach((allowlist) => {
        ...
        summary.append(
          element(
            ...
            allowlist.console_managed
              ? t("crowdsec.allowlists.console", "Console managed")
              : t("crowdsec.allowlists.local", "Locally managed"),
          ),
        );
        ...
        if (!entries.length)
          details.append(
            emptyState(
              t("crowdsec.allowlists.no_entries", "No active entries."),
            ),
          );
        entries.forEach((item) => {
          ...
          const expiration = never
            ? t("crowdsec.allowlists.never", "Never")
            : Number.isFinite(date.getTime())
              ? date.toLocaleString()
              : t("crowdsec.unknown", "Unknown");
          details.append(
            listCard(item.value, [
              [t("crowdsec.allowlists.comment", "Comment"), item.description],
              [t("crowdsec.decision.expires", "Expires"), expiration],
            ]),
          );
        });
  ...
  const renderAllowlistCheck = (payload) => {
    ...
    if (!result || typeof result.allowlisted !== "boolean") {
      renderSectionUnavailable(
        ...
        payload.errors?.allowlist ||
          t("crowdsec.allowlists.unknown", "Allowlist status is unavailable."),
      );
      return;
    }
    ...
    notice.append(
      element(
        "strong",
        "d-block",
        t("crowdsec.allowlists.allowed", "Allowlisted by CrowdSec"),
      ),
    );
  ...
  const renderInvestigation = (payload, connection, ip) => {
    ...
    byId("crowdsec-investigation-title").textContent = t(
      "crowdsec.investigation.result",
      "Investigation for {{ip}}",
      { ip: payload.ip || ip },
    );
    byId("crowdsec-observed-at").textContent = t(
  ...
  connectionFilter.addEventListener("input", applyConnectionFilter);
  ...
  byId("crowdsec-unban-form").addEventListener("submit", async (event) => {
    ...
    try {
      ...
      if (!isCrowdSecRemovalConfirmed(result)) {
        throw new Error(
          t("crowdsec.error.request", "The CrowdSec request failed."),
        );
      }
      if (!pageState.isCurrent(requestState)) return;
