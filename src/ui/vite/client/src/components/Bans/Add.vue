...
<template>
  <div class="col-span-12 grid grid-cols-12">
    <div class="overflow-x-auto col-span-12 grid grid-cols-12">
      <div class="col-span-12 flex flex-col sm:flex-row justify-center absolute items-center mt-2 mb-6 mx-2">
            <ButtonBase
                @click="addItem()"
                color="valid"
                size="normal"
        ...
        <ListBase  v-if="bans.items.length > 0"
              class="min-w-[1050px] h-full col-span-12 pt-28 sm:pt-16"
              :header="addHeader"
              :positions="addPositions"
            >
            <ListItem
              v-for="(item, id) in bans.items" :key="id"
              :class="[id === bans.items.length - 1 ? '' : 'border-b', 'pt-0']"
            >
              <div class="list-content-item-wrap" >
                <SettingsLayout :class="[addPositions[0], 'mr-2']" label="" :name="`add-ip-${id}`">
                  <SettingsInput
                    @inp="(v) => (item.ip = v)"
                    :settings="{
                      id: `add-ip-${id}`,
                ...
                <SettingsLayout :class="[addPositions[1], 'mr-2']" label="" :name="`add-ban-date-deb-${id}`">
                  <SettingsDatepicker
                    @inp="(v) => (item.ip = v)"
                    :settings="{
                ...
                <SettingsLayout :class="[addPositions[2], 'mr-2']" label="" :name="`add-ban-date-end-${id}`">
                  <SettingsDatepicker
                    :settings="{
                ...
                <SettingsLayout :class="[addPositions[3], 'mr-2']" label="" :name="`add-reason-${id}`">
                  <SettingsInput
                    @inp="(v) => (item.reason = v)"
                    :settings="{
                      id: `add-reason-${id}`,
        ...
        <div v-if="bans.items.length > 0" class="col-span-12 flex justify-center mt-4">
            <ButtonBase
        ...
        <BansModalAdd @close="modal.isOpen = false" @sendAdd="sendBans()" :isOpen="modal.isOpen" />
  </div>
  </template>