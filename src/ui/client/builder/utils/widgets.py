...
def container_widget(
    ...
    # List of params that will be add only if not default value
    list_params = [("containerClass", containerClass, ""),("columns", columns, False),("tag", tag, "div")]
    for param in list_params:
        add_key_value(data, param[0], param[1], param[2])
...
def grid_widget(
    ...
    # List of params that will be add only if not default value
    list_params = [("gridClass", gridClass, "items-start")]
    for param in list_params:
        add_key_value(data, param[0], param[1], param[2])
...
def grid_layout_widget(
    ...
    # List of params that will be add only if not default value
    list_params = [("type", type, "card"),("id", id, ""),("title", title, ""),("link", link, ""),("columns", columns, {"pc":12,"tablet":12,"mobile":12}),("gridLayoutClass", gridLayoutClass, "items-start"),("tabId", tabId, "")]
    for param in list_params:
        add_key_value(data, param[0], param[1], param[2])
