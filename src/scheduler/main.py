...
# Initialize healthcheck logger with same configuration
...
# Waits for configuration changes to complete before stopping the scheduler.
def handle_stop(signum, frame):
    ...
    if SCHEDULER is not None:
        if DEBUG_MODE:
            logger.debug("Clearing scheduler before stop")
        SCHEDULER.clear()
    stop(0)
...
# Handle SIGHUP signal to reload the scheduler configuration.
# Saves current configuration to file when database is not read-only.
def handle_reload(signum, frame):
    ...
    try:
        ...
        logger.exception(f"Exception while reloading scheduler : {e}")


signal(SIGHUP, handle_reload)


# Clean up process files and exit with specified status code.
# Removes PID file and healthy status file before terminating.
def stop(status):
    if DEBUG_MODE:
...
# Send configuration files to BunkerWeb instances via API.
# Updates instance status in database and manages API endpoint list.
def send_file_to_bunkerweb(file_path: Path, endpoint: str, 
                          current_logger=logger, *, api_caller: Optional[ApiCaller] = None):
    if DEBUG_MODE:
        current_logger.debug(f"send_file_to_bunkerweb() called with file_path={file_path}, endpoint={endpoint}")
    
    assert SCHEDULER is not None, "SCHEDULER is not defined"
    current_logger.info(f"Sending {file_path} to {'specific' if api_caller else 'all reachable'} BunkerWeb instances ...")
    success, responses = (api_caller or SCHEDULER).send_files(file_path.as_posix(), endpoint, response=True)
    fails = []

    if DEBUG_MODE:
        current_logger.debug(f"File send responses: {responses}")
    ...
    if not IGNORE_FAIL_SENDING_CONFIG:
        for db_instance in SCHEDULER.db.get_instances():
            ...
            if DEBUG_MODE:
                current_logger.debug(f"Instance {db_instance['hostname']} status: {status}")

            ret = SCHEDULER.db.update_instance(db_instance["hostname"], "up" if status == "success" else "down")
            if ret:
                current_logger.error(f"Couldn't update instance {db_instance['hostname']} status to down in the database: {ret}")

            with SCHEDULER_LOCK:
                if status == "success":
                    success = True
                    if index == -1:
                        current_logger.debug(f"Adding {db_instance['hostname']}:{db_instance['port']} to the list of reachable instances")
                    ...
                    current_logger.debug(f"Removing {db_instance['hostname']}:{db_instance['port']} from the list of reachable instances")
                    del SCHEDULER.apis[index]

    if not success:
        current_logger.error(f"Error while sending {file_path} to BunkerWeb instances")
    elif not fails:
        current_logger.info(f"Successfully sent {file_path} folder to reachable BunkerWeb instances")
    elif not IGNORE_FAIL_SENDING_CONFIG:
        current_logger.warning(f"Error while sending {file_path} to some BunkerWeb instances, removing them from the list of reachable instances: {', '.join(fails)}")


# Generate custom configuration files from database and write to filesystem.
# Creates directory structure and sets proper file permissions for configs.
def generate_custom_configs(configs: Optional[List[Dict[str, Any]]] = None, *, 
                           original_path: Union[Path, str] = CUSTOM_CONFIGS_PATH, send: bool = True):
    if DEBUG_MODE:
        logger.debug(f"generate_custom_configs() called with {len(configs) if configs else 0} configs")
    ...
    if configs:
        ...
        for custom_config in configs:
            if DEBUG_MODE:
                logger.debug(f"Processing custom config: {custom_config['name']}")
            ...
            try:
                if custom_config["data"]:
                    ...
                    if DEBUG_MODE:
                        logger.debug(f"Generated custom config file: {tmp_path}")
            except OSError as e:
                logger.exception(f"OSError while generating custom configs \"{custom_config['name']}\"{' for service ' + custom_config['service_id'] if custom_config['service_id'] else ''}: {e}")
                ...
                logger.exception(
                    f"Exception while generating custom configs \"{custom_config['name']}\"{' for service ' + custom_config['service_id'] if custom_config['service_id'] else ''}: {e}"
                )
...
# Generate external or pro plugin files from database to filesystem.
# Extracts tar.gz plugin data and sets executable permissions for plugin files.
def generate_external_plugins(original_path: Union[Path, str] = EXTERNAL_PLUGINS_PATH, *, send: bool = True):
    ...
    if DEBUG_MODE:
        logger.debug(f"generate_external_plugins() called for {'pro' if pro else 'external'} plugins at {original_path}")
            ...
            with suppress(StopIteration, IndexError):
                ...
                with BytesIO() as plugin_content:
                    ...
                    if bytes_hash(plugin_content, algorithm="sha256") == plugins[index]["checksum"]:
                        ...
                        if DEBUG_MODE:
                            logger.debug(f"Plugin {file.name} checksum unchanged, skipping")
                        continue
    ...
    if plugins:
        ...
        for plugin in plugins:
            ...
            if DEBUG_MODE:
                logger.debug(f"Processing plugin: {plugin['id']}")
            ...
            try:
                if plugin["data"]:
                    ...
                    if DEBUG_MODE:
                        logger.debug(f"Successfully extracted and configured plugin: {plugin['id']}")
            except OSError as e:
                logger.exception(f"OSError while generating {'pro ' if pro else ''}external plugins \"{plugin['name']}\": {e}")
                if plugin["method"] != "manual":
                    logger.error(f"Error while generating {'pro ' if pro else ''}external plugins \"{plugin['name']}\": {e}")
            except BaseException as e:
                logger.exception(f"Exception while generating {'pro ' if pro else ''}external plugins \"{plugin['name']}\": {e}")
...
# Generate cache files from database to filesystem with proper directory structure.
# Extracts compressed cache data and sets appropriate file permissions.
def generate_caches():
    if DEBUG_MODE:
    ...
    if DEBUG_MODE:
        logger.debug(f"Processing {len(job_cache_files)} cache files")
    ...
    for job_cache_file in job_cache_files:
        ...
        if DEBUG_MODE:
            logger.debug(f"Processing cache file: {job_cache_file['file_name']}")
        ...
        try:
            ...
            logger.exception(f"Exception while restoring cache file {job_cache_file['file_name']} :\n{e}")
...
# Run the configuration generator to create nginx configuration files.
# Executes the BunkerWeb config generator and copies variables file to temp location.
def generate_configs(current_logger=logger) -> bool:
    if DEBUG_MODE:
        current_logger.debug("generate_configs() called")
    ...
    if proc.returncode != 0:
        current_logger.error("Config generator failed, configuration will not work as expected...")
        return False

    copy(NGINX_VARIABLES_PATH.as_posix(), NGINX_TMP_VARIABLES_PATH.as_posix())
    
    if DEBUG_MODE:
        current_logger.debug("Config generation completed successfully")
    
    return True


# Perform health checks on BunkerWeb instances and manage their status.
# Sends configuration to loading instances and updates database status.
def healthcheck_job():
    if DEBUG_MODE:
    ...
    if APPLYING_CHANGES.is_set():
        if DEBUG_MODE:
            healthcheck_logger.debug("Applying changes in progress, skipping healthcheck")
        return
    ...
    for db_instance in SCHEDULER.db.get_instances():
        if DEBUG_MODE:
            healthcheck_logger.debug(f"Checking instance: {db_instance['hostname']}:{db_instance['port']}")
        ...
        logger.exception(f"Exception while executing scheduler")
        stop(1)
                        env["TZ"] = tz
            ...
            healthcheck_logger.exception(f"Exception while checking instance {bw_instance.endpoint}: {e}")
            for i, api in enumerate(SCHEDULER.apis):
                if api.endpoint == bw_instance.endpoint:
                    healthcheck_logger.debug(f"Removing {bw_instance.endpoint} from the list of reachable instances")
                    del SCHEDULER.apis[i]
    ...
    if DEBUG_MODE:
        healthcheck_logger.debug("healthcheck_job() completed")


# Create a backup of the current working configuration for failover purposes.
...
def backup_failover():
    ...
    try:
        ...
        for src, dst_name in (
            ...
            if DEBUG_MODE:
                logger.debug(f"Copying {src} to failover path as {dst_name}")
        ...
        logger.exception(f"Failed to initialize failover backup: {e}")
    finally:
...
if __name__ == "__main__":
    try:
        if DEBUG_MODE:
            logger.debug("Starting BunkerWeb scheduler")
        ...
        if DEBUG_MODE:
            logger.debug(f"Parsed arguments: variables={args.variables}")
        ...
        if tmp_variables_path.is_file():
            if DEBUG_MODE:
                logger.debug(f"Loading variables from {tmp_variables_path}")
        ...
        while not ready:
            ...
            if isinstance(db_metadata, str) or not db_metadata["is_initialized"]:
                ...
                ready = True
                if DEBUG_MODE:
                    logger.debug("Database is ready and initialized")
                continue
            sleep(5)
        ...
        if DEBUG_MODE:
            logger.debug(f"Loaded environment with {len(env)} variables")
        ...
        # Check if any custom config has been created by the user.
        # Compares filesystem configs with database and saves new manual configs.
        def check_configs_changes():
            ...
            if DEBUG_MODE:
                logger.debug(f"Custom configs changes detected: {changes}")
            ...
            if changes:
                try:
                    ...
                    logger.exception(f"Error while saving custom configs to database: {e}")

            generate_custom_configs(SCHEDULER.db.get_custom_configs())

        # Check if any external or pro plugin has been added by the user.
        # Compares filesystem plugins with database and saves new manual plugins.
        def check_plugin_changes(_type: Literal["external", "pro"] = "external"):
            if DEBUG_MODE:
                logger.debug(f"check_plugin_changes() called for {_type} plugins")
                ...
                with BytesIO() as plugin_content:
                    ...
                    with suppress(StopIteration, IndexError):
                        ...
                        if checksum == db_plugins[index]["checksum"] or db_plugins[index]["method"] != "manual":
                            if DEBUG_MODE:
                                logger.debug(f"Plugin {common_data['id']} unchanged or not manual, skipping")
                            continue
            ...
            if tmp_external_plugins:
                ...
                if DEBUG_MODE:
                    logger.debug(f"{_type} plugins changes detected: {changes}")
                ...
                if changes:
                    try:
                        ...
                        logger.exception(f"Error while saving {_type} plugins to database: {e}")
                else:
        ...
        if db_metadata["pro_plugins_changed"] or db_metadata["external_plugins_changed"]:
            ...
            if DEBUG_MODE:
                logger.debug(f"Plugins changed - pro: {db_metadata['pro_plugins_changed']}, external: {db_metadata['external_plugins_changed']}")
            ...
            if SCHEDULER.db.readonly:
                ...
                if proc.returncode != 0:
                    ...
                    logger.debug("Config saver for plugins settings completed successfully")
        ...
        while True:
            ...
            if CONFIG_NEED_GENERATION:
                if DEBUG_MODE:
                    logger.debug("Generating configs")
            ...
            try:
                ...
                if SCHEDULER.apis:
                    ...
                    if DEBUG_MODE:
                        logger.debug("Sending reload request to all APIs")
                    ...
                    for db_instance in SCHEDULER.db.get_instances():
                        ...
                        if DEBUG_MODE:
                            logger.debug(f"Instance {db_instance['hostname']} reload status: {status}")
                ...
                logger.exception(f"Exception while reloading after running jobs once scheduling : {e}")

            try:
                SCHEDULER.db.set_metadata({"failover": not success, "failover_message": failover_message})
            except BaseException as e:
                logger.exception(f"Error while setting failover to true in the database: {e}")
            ...
            try:
                if not success and reachable:
                    ...
                    if (
                        ...
                        if DEBUG_MODE:
                            logger.debug("Executing failover to last working configuration")
                ...
                logger.exception(f"Exception while executing failover logic : {e}")
            ...
            try:
                ...
                logger.exception(f"Error while setting changes to checked in the database: {e}")
            ...
            if scheduler_first_start:
                try:
                    ...
                    if ret == "The database is read-only, the scheduler first start will not be saved":
                    ...
                    logger.exception(f"Error while setting the scheduler first start : {e}")
                finally:
            ...
            while RUN and not NEED_RELOAD:
                try:
                    ...
                    if DEBUG_MODE and changes != old_changes:
            ...
            if NEED_RELOAD:
                if DEBUG_MODE:
                    logger.debug(f"Reload needed, changes: {changes}")
