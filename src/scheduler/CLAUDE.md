...
- Checks each instance via `GET /instances/{hostname}/health`, which forwards the instance's own state: `ok`, `loading`, `reloading` or `needs_config`. A failure there means unreachable — the instance answers 200 in every state
- **`loading` and `needs_config` both earn a re-push and are not the same thing.** `loading` means there is no configuration, so the nine plugins below are inactive: a real exposure. `needs_config` is a restart that kept its configuration, is enforcing all of it, and simply wants a fresh one — set by the entrypoint as `/var/tmp/bunkerweb_needs_config` and cleared by `POST /confs`. Do not merge their log messages: telling an operator that access controls are inactive when they are enforcing sends them chasing a bypass that does not exist
- Dispatches `push-configs` when an instance transitions `down`/`failover` → `up`, **or** when a reachable instance reports `loading`. The second case is the one a short container restart needs: the instance comes back reachable but keeps `IS_LOADING=yes` until something pushes it a configuration
- **`IS_LOADING` is a security state, not a telemetry one.** Nine core plugins gate `is_needed()` on it — `mtls.lua:47`, `authbasic.lua:207`, `blacklist.lua:69`, `limit.lua:98`, plus greylist, whitelist, dnsbl, bunkernet and robotstxt — so an instance holding it serves traffic enforcing no client certificates, no basic auth, no blacklist and no rate limit, while answering healthchecks as `up`. The timer-driven work stops too. ModSecurity/CRS, antibot, country and the bans path (`access-lua.conf`) do **not** read the flag and keep enforcing, so this is a partial bypass, not a disabled WAF
- The loading re-push is therefore retried, but **bounded**: `LOADING_FAST_RETRIES` passes in a row, then one every `LOADING_SLOW_RETRY_EVERY`. Retrying at all matters because `run_single` can report success without queueing anything (a read-only database does exactly that), so one attempt proves nothing; bounding it matters because the instance is marked `up` before this branch, which makes each attempt a full `push-configs` — render, upload, fleet-wide reload — and `api.lua`'s `/health` fails toward `loading`, so a datastore hiccup alone lands here. `LOADING_INSTANCES` maps hostname → consecutive loading healthchecks and drives both the cadence and the log level
