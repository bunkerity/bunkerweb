...
- **`POST /instances/enroll` is the one route in `routers/instances.py` with no `Depends(guard)`.** It is the only way an instance can get a credential before it has one to authenticate with; it is rate-limited like `/auth` instead. Every other enrollment endpoint (`/{hostname}/enroll`, `/rotate`, `/revoke`) is guarded normally, needing `instances_enroll`/`instances_rotate`.
- **The reserved `default-server` service** is a normal row in `bw_services` as far as `routers/services.py` is concerned (present only when `MULTISITE=yes`), refused only by explicit predicate checks rather than by a separate code path — a new services endpoint that skips them will silently accept a write the others reject. Two predicates, not one, and they disagree on purpose: `is_reserved_default_server()` (delete, rename-away, draft) checks id **and** `method="wizard"`, so a pre-1.7 row that merely happened to be named `default-server` keeps an exit via rename/delete; `is_default_server()` (rename-onto, `SERVER_TYPE` in `variables`, create) checks the id alone. Both live in `src/common/utils/default_server.py`; the refusal response itself is `_reserved_refusal()` in `routers/services.py`.
