...
def remove(connection_id: str, decision_id: int, selection: DecisionSelection, request: Request):
    ...
    try:
        ...
        LOGGER.warning(
            "CrowdSec removal actor=%r connection=%r decision=%s target=%r outcome=error status=%s",
            actor,
            connection_id,
            decision_id,
            selection.value,
        ...
        raise
    LOGGER.info(
        "CrowdSec removal actor=%r connection=%r decision=%s target=%r outcome=removed propagation=pending", actor, connection_id, decision_id, selection.value
    )
    return result
