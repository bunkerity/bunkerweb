...
async def update_config(
    ...
    if "database is locked" in resp or "file is not a database" in resp:
        ...
        CORE_CONFIG.logger.warning(
            f"Can't save config to database : Database is locked or had trouble handling the request, retry in {retry_in} seconds"
        )
        return JSONResponse(
        ...
        return JSONResponse(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, content={"message": resp}
        )
...
async def update_global_config(
    ...
    if "database is locked" in resp or "file is not a database" in resp:
        retry_in = str(uniform(1.0, 5.0))
        CORE_CONFIG.logger.warning(
            f"Can't save global config to database : Database is locked or had trouble handling the request, retry in {retry_in} seconds"
        )
        return JSONResponse(
            status_code=status.HTTP_503_SERVICE_UNAVAILABLE,
            content={
...
async def update_service_config(
    ...
    if resp == "not_found":
        ...
        retry_in = str(uniform(1.0, 5.0))
        CORE_CONFIG.logger.warning(
            f"Can't save config for service {service_name} to database : Database is locked or had trouble handling the request, retry in {retry_in} seconds"
        )
        return JSONResponse(
            status_code=status.HTTP_503_SERVICE_UNAVAILABLE,
            content={
        ...
        CORE_CONFIG.logger.error(
            f"Can't save service {service_name} config to database : {resp}"
        )
        return JSONResponse(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, content={"message": resp}
        )
...
async def delete_service_config(
    ...
    if resp == "not_found":
        ...
        retry_in = str(uniform(1.0, 5.0))
        CORE_CONFIG.logger.warning(
            f"Can't delete service {service_name} from the database : Database is locked or had trouble handling the request, retry in {retry_in} seconds"
        )
        return JSONResponse(
            status_code=status.HTTP_503_SERVICE_UNAVAILABLE,
            content={
        ...
        CORE_CONFIG.logger.error(
            f"Can't delete service {service_name} from the database : {resp}"
        )
        return JSONResponse(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, content={"message": resp}
        )
