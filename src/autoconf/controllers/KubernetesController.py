...
class KubernetesController(Controller):
    ...
    def _is_valid_reverse_proxy_url(self, url: str) -> bool:
        ...
        # No regex means no opinion, exactly like a setting that declares none. `_settings` itself
        # is always present (`Config.__init__`, reached through `Controller`'s `super()`), and
        # `_get_setting_groups` already reads it unguarded.
