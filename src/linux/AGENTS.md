...
Six units, all `Type=simple` with `Restart=always` (the heavy worker adds `RestartPreventExitStatus=3`: exit 3 is its own deliberate "nothing to consume" stop, not a crash):
...
| `bunkerweb-worker-heavy.service` | `scripts/bunkerweb-worker.sh` (`start heavy`) | `worker-heavy.pid`      | Second Celery executor pinned to the `heavy` queue (`push-configs`, certbot, plugin downloads, `async` plugin jobs), so a long plugin job never delays the default lane. Same broker dependency as the default worker; enabled and restarted alongside it |
