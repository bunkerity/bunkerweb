...
# Fedora ships nginx from its own repos (1.30.2), one patch release behind the
# source-built target (1.30.3). Dynamic modules must be compiled against the
# matching nginx source tree or they fail to load, so select the 1.30.2 tree here.
if [ "$OS" = "fedora" ] ; then
	NGINX_SRC_DIR="/tmp/bunkerweb/deps/src/nginx-1.30.2"
else
