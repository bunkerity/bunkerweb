...
# Fedora 43 ships nginx from its own repos (1.30.4), one patch release behind the
# source-built target (1.30.5). Dynamic modules must be compiled against the
# matching nginx source tree or nginx refuses to load them, so select the 1.30.4 tree there.
if [ "$OS" = "fedora" ] && [ "$OS_VERSION" = "43" ] ; then
	NGINX_SRC_DIR="/tmp/bunkerweb/deps/src/nginx-1.30.4"
else
