...
ngx_int_t
ngx_stream_lua_proxy_ssl_verify_handler_file(ngx_stream_lua_request_t *r,
    ngx_stream_lua_srv_conf_t *lscf, lua_State *L)
{
    ngx_int_t           rc;

    rc = ngx_stream_lua_cache_loadfile(r->connection->log, L,
                                       lscf->ups.proxy_ssl_verify_src.data,
                                       lscf->ups.proxy_ssl_verify_src_key);
    if (rc != NGX_OK) {
        return rc;
    }
    ...
    return ngx_stream_lua_proxy_ssl_verify_by_chunk(L, r);
}


ngx_int_t
ngx_stream_lua_proxy_ssl_verify_handler_inline(ngx_stream_lua_request_t *r,
    ngx_stream_lua_srv_conf_t *lscf, lua_State *L)
{
    ngx_int_t           rc;
    ...
    rc = ngx_stream_lua_cache_loadbuffer(r->connection->log, L,
                                         lscf->ups.proxy_ssl_verify_src.data,
                                         lscf->ups.proxy_ssl_verify_src.len,
                                         lscf->ups.proxy_ssl_verify_src_key,
                                         "=proxy_ssl_verify_by_lua");
    if (rc != NGX_OK) {
        return rc;
...
char *
ngx_stream_lua_proxy_ssl_verify_by_lua_block(ngx_conf_t *cf, ngx_command_t *cmd,
    void *conf)
{
    char        *rv;
    ngx_conf_t   save;
    ...
    save = *cf;
    cf->handler = ngx_stream_lua_proxy_ssl_verify_by_lua;
    ...
    void *conf)
{
    ...
    if (lscf->ups.proxy_ssl_verify_handler) {
        return "is duplicate";
    }
    ...
    lscf->ups.proxy_ssl_verify_handler =
        (ngx_stream_lua_srv_conf_handler_pt) cmd->post;

    if (cmd->post == ngx_stream_lua_proxy_ssl_verify_handler_file) {
        ...
        lscf->ups.proxy_ssl_verify_src.data = name;
        lscf->ups.proxy_ssl_verify_src.len = ngx_strlen(name);
        ...
        lscf->ups.proxy_ssl_verify_src_key = p;
        ...
        *p = '\0';
        ...
        lscf->ups.proxy_ssl_verify_src = value[1];

        p = ngx_palloc(cf->pool,
                       sizeof("proxy_ssl_verify_by_lua") +
        ...
        p = ngx_copy(p, "proxy_ssl_verify_by_lua",
...
ngx_stream_lua_proxy_ssl_verify_handler(X509_STORE_CTX *x509_store, void *arg)
{
    ...
    if (lscf->ups.upstream_skip_openssl_default_verify == 0) {
        ngx_log_debug0(NGX_LOG_DEBUG_STREAM, c->log, 0,
    ...
    cln->handler = ngx_stream_lua_proxy_ssl_verify_done;
    cln->data = cctx;
    ...
    *cctx->cleanup = ngx_stream_lua_proxy_ssl_verify_aborted;
...
static void
ngx_stream_lua_proxy_ssl_verify_aborted(void *data)
{
    ngx_stream_lua_ssl_ctx_t  *cctx = data;
...
static ngx_int_t
ngx_stream_lua_proxy_ssl_verify_by_chunk(lua_State *L,
    ngx_stream_lua_request_t *r)
{
    int                              co_ref;
    ngx_int_t                        rc;
    ...
    ctx->context = NGX_STREAM_LUA_CONTEXT_PROXY_SSL_VERIFY;
