...
| `LETS_ENCRYPT_PROFILE`                      | `classic` | multisite | no       | **Profil de certificat (Let's Encrypt uniquement) :** Sélectionnez le profil à utiliser. Options : `classic` (général, max 100 SAN), `tlsserver` (optimisé TLS, max 25 SAN), ou `shortlived` (~7 jours, max 25 SAN). Ignoré si `LETS_ENCRYPT_SERVER=zerossl`.                                                               |
| `LETS_ENCRYPT_CUSTOM_PROFILE`               |           | multisite | no       | **Profil de certificat personnalisé (Let's Encrypt uniquement) :** Saisissez un profil personnalisé si votre serveur ACME le supporte. Remplace `LETS_ENCRYPT_PROFILE` s'il est défini. Ignoré si `LETS_ENCRYPT_SERVER=zerossl`.                                                                                              |
