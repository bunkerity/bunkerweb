...
7.  **Seleccione el perfil del certificado (solo Let's Encrypt):** Elija su perfil de certificado preferido utilizando el ajuste `LETS_ENCRYPT_PROFILE` (classic, tlsserver o shortlived). Este ajuste no tiene efecto cuando se usa ZeroSSL.
...
!!! tip "Perfiles de Certificado (solo Let's Encrypt)"
    Let's Encrypt proporciona diferentes perfiles de certificado para diferentes casos de uso. **Estos perfiles solo se aplican cuando `LETS_ENCRYPT_SERVER=letsencrypt`. Son ignorados por ZeroSSL.**

    - **classic**: Certificados de propósito general, validez de 90 días, máx. 100 SAN (predeterminado)
    - **tlsserver**: Validez de 90 días, carga útil reducida (sin Common Name ni Subject Key ID), máx. 25 SAN — recomendado para configuraciones totalmente automatizadas
    - **shortlived**: Validez de ~7 días (160 horas), sin información de revocación CRL (certificados aún más pequeños), máx. 25 SAN — requiere renovación automática fiable
...
!!! info "Disponibilidad de perfiles"
    Tenga en cuenta que los perfiles `tlsserver` y `shortlived` pueden no estar disponibles en todos los entornos o con todos los clientes ACME en este momento. El perfil `classic` tiene la compatibilidad más amplia y se recomienda para la mayoría de los usuarios. Si un perfil seleccionado no está disponible, el sistema volverá automáticamente al perfil `classic`. Existe un cuarto perfil `tlsclient`, pero Let's Encrypt lo discontinuará el 13 de mayo de 2026 y no debe usarse. **Los perfiles de certificado no son compatibles con ZeroSSL — `LETS_ENCRYPT_PROFILE` y `LETS_ENCRYPT_CUSTOM_PROFILE` se ignoran cuando `LETS_ENCRYPT_SERVER=zerossl`.**
...
| `LETS_ENCRYPT_DNS_PROPAGATION`              | `default`         | multisite | no       | **Propagación de DNS:** El tiempo de espera para la propagación de DNS en segundos. Con `default`, BunkerWeb calcula automáticamente el tiempo de espera según el número de dominios (10 segundos × número de dominios), ya que certbot agrega registros TXT secuencialmente. Establezca un valor explícito en segundos para anular este comportamiento.                     |
...
| `LETS_ENCRYPT_PROFILE`                      | `classic`         | multisite | no       | **Perfil de certificado (solo Let's Encrypt):** Seleccione el perfil de certificado a utilizar. Opciones: `classic` (propósito general), `tlsserver` (optimizado para servidores TLS) o `shortlived` (certificados de 7 días). Se ignora cuando `LETS_ENCRYPT_SERVER=zerossl`.                                                                                              |
| `LETS_ENCRYPT_CUSTOM_PROFILE`               |                   | multisite | no       | **Perfil de certificado personalizado (solo Let's Encrypt):** Ingrese un perfil de certificado personalizado si su servidor ACME admite perfiles no estándar. Esto anula `LETS_ENCRYPT_PROFILE` si está configurado. Se ignora cuando `LETS_ENCRYPT_SERVER=zerossl`.                                                                                                        |
...
| `LETS_ENCRYPT_HOSTNAME_CHECK`               | `no`          | multisite | no       | **Verificar registros DNS del nombre de host:** Cuando se establece en `yes`, valida que cada nombre de host tenga un registro DNS A, AAAA o CNAME válido antes de solicitar un certificado con el desafío HTTP. Deshabilitado por defecto ya que las configuraciones DNS internas o split-horizon pueden no resolver nombres de host públicos desde dentro del contenedor. |
...
- **Deshabilitado por defecto:** El ajuste `LETS_ENCRYPT_HOSTNAME_CHECK` está deshabilitado por defecto ya que las configuraciones DNS internas o split-horizon pueden no resolver nombres de host públicos desde dentro del contenedor, causando falsos negativos.
- **Habilitar la verificación:** Establezca `LETS_ENCRYPT_HOSTNAME_CHECK` en `yes` para validar los registros DNS de cada dominio antes del desafío HTTP. Si un dominio no resuelve a una dirección IP o CNAME válido, se muestra un error y la solicitud de certificado se omite para ese dominio.
...
LETS_ENCRYPT_HOSTNAME_CHECK: "no"  # (predeterminado)
```
...
!!! info "Información y comportamiento"
    ...
    - Cuando `LETS_ENCRYPT_DNS_PROPAGATION` se establece en `default`, BunkerWeb escala el tiempo de espera automáticamente: 10 segundos por dominio en el certificado (p. ej., 2 dominios = 20 segundos). Establezca un valor explícito para anular este comportamiento.
