...
| `LETS_ENCRYPT_PROFILE`                      | `classic`         | multisite | no       | **Perfil de certificado (solo Let's Encrypt):** Seleccione el perfil a utilizar. Opciones: `classic` (propósito general, máx. 100 SAN), `tlsserver` (optimizado para servidores TLS, máx. 25 SAN) o `shortlived` (~7 días, máx. 25 SAN). Se ignora cuando `LETS_ENCRYPT_SERVER=zerossl`.                                                                                   |
| `LETS_ENCRYPT_CUSTOM_PROFILE`               |                   | multisite | no       | **Perfil de certificado personalizado (solo Let's Encrypt):** Ingrese un perfil personalizado si su servidor ACME admite perfiles no estándar. Esto anula `LETS_ENCRYPT_PROFILE` si está configurado. Se ignora cuando `LETS_ENCRYPT_SERVER=zerossl`.                                                                                                                       |
