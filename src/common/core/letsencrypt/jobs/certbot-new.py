    ...
    for conf_file in sorted(renewal_dir.glob("*.conf")):
        ...
        try:
            ...
            if key_type.lower() == "ecdsa":
                ...
                # Convert curve name to readable format
                curve_map = {"secp256r1": "P-256", "secp384r1": "P-384"}
                key_details = curve_map.get(curve, curve)
                key_type_display = f"ECDSA ({key_details})"
...
def certbot_delete(service: str, cmd_env: Dict[str, str] = None) -> int:
    ...
    try:
        return run_process_with_timeout(command, 360, LOGGER_CERTBOT, cmd_env)
    except TimeoutExpired:
        LOGGER.error(f"Certificate deletion for {service} exceeded 6 minute timeout")
        return 1
...
def certbot_new(
    ...
    if config["challenge"] == "dns":
        ...
        # Log credentials with values masked to help diagnose format issues without exposing secrets
        masked_credentials = "\n".join(f"{line.split(' = ')[0]} = ***" if " = " in line else line for line in credentials.decode("utf-8", errors="replace").splitlines())
        LOGGER_CERTBOT.debug(f"[Service: {service}] Credentials content:\n{masked_credentials}")
    ...
    if config["force_renew"]:
        # Use --force-renewal flag instead of deleting the certificate first.
        # Previously we called certbot_delete() then re-ran certbot, which risked
        # a window where no valid certificate existed. With --force-renewal certbot
        # obtains a new cert before replacing the old one (safer atomic swap).
...
try:
    ...
    activated_services = {s: c for s, c in services.items() if c["activated"]}
    deactivated_services = {s: c for s, c in services.items() if not c["activated"]}
    if deactivated_services:
        LOGGER.debug(f"Services with Let's Encrypt disabled: {', '.join(deactivated_services.keys())}")

    if not activated_services:
        LOGGER.info("No services uses Let's Encrypt, skipping generation of new certificates...")
        sys_exit(0)

    # Clear main letsencrypt.log only once at job start, subsequent certificates will append
    prepare_logs_dir(LOGS_DIR, LOGGER, clear_main_log=True)
    ...
    for service, config in services.items():
        ...
        if not config["activated"]:
            LOGGER_CERTBOT.debug(f"[Service: {service}] Let's Encrypt is not activated. Set AUTO_LETS_ENCRYPT=yes to enable certificate generation for this service.")
            continue
