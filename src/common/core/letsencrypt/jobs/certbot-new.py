...
JOB = Job(API(getenv("API_ADDR", ""), "job-certbot-new"), getenv("CORE_TOKEN", None))
status = 0
...
try:
    ...
    tgz = JOB.get_cache("folder.tgz", job_name="certbot-renew")
    if tgz:
    ...
    # Put tgz in cache
    cached, err = JOB.cache_file("folder.tgz", bio.read(), job_name="certbot-renew")
