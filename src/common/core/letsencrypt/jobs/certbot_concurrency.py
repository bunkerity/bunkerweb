...
def select_account_id(accounts_root: Path, staging: bool, email: str, acme_server_url: str = "") -> Optional[str]:
    """Return the certbot account ID to pin with ``--account``.

    Replaces the old per-server-dir scan with a URL-derived path so the correct
    account directory is found regardless of how many ACME servers are configured.
    Falls back to the most-recently-modified account when no email match is found,
    which keeps behaviour identical to the previous implementation.
    """
...
def _account_exists(accounts_root: Path, staging: bool, email: str, acme_server_url: str = "") -> bool:
    """Check whether a certbot account already exists for the given email and ACME server.

    Replaces the old nested server-dir / directory-subdir scan with the URL-derived
    path from ``_find_accounts``, so it correctly handles ZeroSSL (``/v2/DV90``) and
    any future server whose URL path has more than one component.
    """
    accts_dir = _find_accounts(accounts_root, acme_server_url, staging)
    if not accts_dir.is_dir():
        return False
...
def ensure_accounts(
    ...
    for staging, email in sorted(requests, key=itemgetter(0, 1)):
        # Pass the full ACME directory URL so _account_exists uses the deterministic
        # URL-derived path instead of the legacy name-heuristic fallback.
