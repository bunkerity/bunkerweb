...
# This allows DNS providers to accept credentials under multiple field names
# for better compatibility with different configuration formats.
# Args: field_map - Dictionary mapping canonical field names to list of aliases
# Returns: Configured model_validator function
    ...
    def validator(cls, values):
        ...
        for field, aliases in field_map.items():
            for alias in aliases:
                if alias in values:
                    if getenv("LOG_LEVEL", "INFO").upper() == "DEBUG":
                        print(f"DEBUG: Mapping alias '{alias}' to field '{field}'")
...
class Provider(BaseModel):
    ...
    # Provides common functionality for credential formatting and file type handling.
    # All DNS provider classes inherit from this base class.

    model_config = ConfigDict(extra="ignore")

    # Return the formatted credentials to be written to a file.
    # Default implementation creates INI-style key=value format.
    # Returns: bytes - UTF-8 encoded credential content
    ...
    # Returns: Literal type indicating file extension
    @staticmethod
...
class CloudflareProvider(Provider):
    # Supports both API token (recommended) and legacy email/API key authentication.
    # Requires either api_token OR both email and api_key for authentication.
    ...
    # Only includes non-empty credential fields to avoid cluttering output.
    # Returns: bytes - UTF-8 encoded credential content
    def get_formatted_credentials(self) -> bytes:
        if getenv("LOG_LEVEL", "INFO").upper() == "DEBUG":
    ...
    # Raises: ValueError if neither authentication method is complete
    ...
    def validate_cloudflare_credentials(self):
        ...
        if getenv("LOG_LEVEL", "INFO").upper() == "DEBUG":
            print(f"DEBUG: Cloudflare credential validation:")
    ...
    _validate_aliases = alias_model_validator(
        {
            ...
            "dns_dnsmadeeasy_secret_key": (
                "dns_dnsmadeeasy_secret_key", "dnsmadeeasy_secret_key", "secret_key"
            ),
        }
    )
...
class GoogleProvider(Provider):
    ...
    _validate_aliases = alias_model_validator(
        {
            ...
            "private_key_id": (
                "private_key_id", "google_private_key_id", "dns_google_private_key_id"
            ),
            "private_key": (
    ...
    # Returns: bytes - UTF-8 encoded JSON content
    def get_formatted_credentials(self) -> bytes:
        if getenv("LOG_LEVEL", "INFO").upper() == "DEBUG":
    ...
    # Return the file type that the credentials should be written to.
    # Google provider requires JSON format for service account credentials.
    # Returns: Literal type indicating JSON file extension
    @staticmethod
...
class IonosProvider(Provider):
    # Requires prefix and secret for authentication, with configurable endpoint.
    ...
    _validate_aliases = alias_model_validator(
        {
            ...
            "dns_ovh_application_key": (
                "dns_ovh_application_key", "ovh_application_key", "application_key"
            ),
...
class Route53Provider(Provider):
    ...
    # Returns: bytes - UTF-8 encoded environment variable format
    def get_formatted_credentials(self) -> bytes:
        if getenv("LOG_LEVEL", "INFO").upper() == "DEBUG":
    ...
    # Returns: Literal type indicating env file extension
    @staticmethod
    ...
    _validate_aliases = alias_model_validator(
        {
            "dns_sakuracloud_api_token": (
                "dns_sakuracloud_api_token", "sakuracloud_api_token", "api_token"
            ),
            "dns_sakuracloud_api_secret": (
                "dns_sakuracloud_api_secret", "sakuracloud_api_secret", "api_secret"
            ),
        }
    )
...
class NjallaProvider(Provider):
    ...
    _validate_aliases = alias_model_validator(
        {
            "dns_njalla_token": (
                "dns_njalla_token", "njalla_token", "token", "api_token", "auth_token"
            ),
        }
    )
...
class WildcardGenerator:
    ...
    # Args: group - Group identifier for these domains
    ...
    def extend(self, group: str, domains: List[str], email: str, staging: bool = False):
        if getenv("LOG_LEVEL", "INFO").upper() == "DEBUG":
            ...
            print(f"DEBUG: Staging mode: {staging}, Email: {email}")
            print(f"DEBUG: Domains: {domains}")
        ...
        if getenv("LOG_LEVEL", "INFO").upper() == "DEBUG":
            print(f"DEBUG: Added {domains_added} domains to {env_type} environment")
            total_staging = len(self.__domain_groups[group]["staging"])
            total_prod = len(self.__domain_groups[group]["prod"])
            print(f"DEBUG: Group '{group}' now has {total_staging} staging, {total_prod} prod domains")
    ...
    # Creates optimized wildcard certificates that cover multiple subdomains.
    # Args: staging - Whether to generate wildcards for staging environment
    def __generate_wildcards(self, staging: bool = False):
        ...
        if getenv("LOG_LEVEL", "INFO").upper() == "DEBUG":
            print(f"DEBUG: Generated wildcards for {wildcards_generated} domains")

    # Convert a domain to wildcard patterns and add to the wildcards collection.
    # Determines optimal wildcard patterns based on domain structure.
    # Args: domain - Domain to process
    #       group - Group identifier  
    #       env_type - Environment type (staging or prod)
    def __add_domain_wildcards(self, domain: str, group: str, env_type: str):
        if getenv("LOG_LEVEL", "INFO").upper() == "DEBUG":
            print(f"DEBUG: Processing domain '{domain}' for wildcards")
        ...
        if len(parts) > 2:
            ...
            if getenv("LOG_LEVEL", "INFO").upper() == "DEBUG":
                print(f"DEBUG: Added wildcard '{wildcard_domain}' and base '{base_domain}'")
        else:
    ...
    # Returns: Dictionary of group data with formatted wildcard domains
    ...
    # Args: domains - List of domains to process
    # Returns: List of extracted wildcard domains
    @staticmethod
    def extract_wildcards_from_domains(domains: List[str]) -> List[str]:
        ...
        if getenv("LOG_LEVEL", "INFO").upper() == "DEBUG":
            print(f"DEBUG: Generated {len(result)} wildcard patterns: {result}")
        
        return result

    # Extract the base domain from a domain name.
    # Removes wildcard prefix if present to get the actual domain.
    # Args: domain - Input domain name
    # Returns: Base domain (without wildcard prefix if present)
    @staticmethod
    def get_base_domain(domain: str) -> str:
        ...
        if getenv("LOG_LEVEL", "INFO").upper() == "DEBUG":
            if domain != base:
                print(f"DEBUG: Extracted base domain '{base}' from '{domain}'")
        
        return base

    # Generate a consistent group name for wildcards.
    # Creates a unique identifier for grouping related wildcard certificates.
    # Args: domain - The domain name
    ...
    def create_group_name(domain: str, provider: str, challenge_type: str, 
        ...
        if getenv("LOG_LEVEL", "INFO").upper() == "DEBUG":
            ...
            print(f"DEBUG: Staging: {staging}, Profile: {profile}")
            print(f"DEBUG: Content hash: {content_hash[:10]}...")
