...
try:
    ...
    for instance, config in configs.items():
        # Multisite case
        configs[instance]["BLACKLIST_ACTIVATED"] = False
        if config.get("MULTISITE", "no") == "yes":
            for first_server in config.get("SERVER_NAME", "").split(" "):
                if (
                    config.get(
                        f"{first_server}_USE_BLACKLIST",
                        config.get("USE_BLACKLIST", "yes"),
                    )
                    == "yes"
                ):
                    blacklist_instances.append(instance)
                    break
        # Singlesite case
        elif config.get("USE_BLACKLIST", "yes") == "yes":
            blacklist_instances.append(instance)

    if not blacklist_instances:
    ...
    for instance in blacklist_instances:
        # Our urls data
        urls = {
            "IP": [],
            "RDNS": [],
        ...
        for kind in urls:
            for url in configs[instance].get(f"BLACKLIST_{kind}_URLS", "").split(" "):
        ...
        for instance_urls in instances_urls:
            if urls == instances_urls[instance_urls]:
                same_as = instance_urls
                break

        instance_blacklist_path = blacklist_path.joinpath(
            same_as or (instance if instance != "127.0.0.1" else "")
        )
        instance_blacklist_path.mkdir(parents=True, exist_ok=True)
        ...
        if same_as:
            if instance != "127.0.0.1":
                sent, res = send_cache_to_api(instance_blacklist_path, instance_api)
                if not sent:
                    logger.error(f"Error while sending blacklist to API : {res}")
                    sys_exit(2)
                logger.info(res)
            continue
        ...
        for kind in kinds_fresh:
            if not is_cached_file(
                instance_blacklist_path.joinpath(f"{kind}.list"),
                "hour",
                instance,
                db,
            ):
        ...
        for kind, urls_list in urls.items():
            ...
            for url in urls_list:
                try:
                    ...
                    instance_tmp_blacklist_path = tmp_blacklist_path.joinpath(
                        instance if instance != "127.0.0.1" else ""
                    )
                    instance_tmp_blacklist_path.mkdir(parents=True, exist_ok=True)
                    instance_tmp_blacklist_path.joinpath(f"{kind}.list").write_bytes(
                        content
                    )
                    ...
                    new_hash = file_hash(
                        instance_tmp_blacklist_path.joinpath(f"{kind}.list")
                    )
                    old_hash = cache_hash(
                        instance_blacklist_path.joinpath(f"{kind}.list"),
                        instance,
