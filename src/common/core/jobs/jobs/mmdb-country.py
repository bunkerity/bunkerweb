...
logger = setup_logger(
    title="jobs-mmdb-country",
    log_file_path="/var/log/bunkerweb/jobs.log"
)

logger.debug("Debug mode enabled for jobs-mmdb-country")
...
# Request MMDB country file information from db-ip.com to check for updates.
def request_mmdb() -> Optional[Response]:
    logger.debug("request_mmdb() called for country database")
    try:
        max_retries = 3
        retry_count = 0
        logger.debug("Starting country version check request to db-ip.com")
        ...
        logger.debug("Country version check request completed successfully")
        return response
    except RequestException as e:
        logger.exception("RequestException during country version check")
        return None


try:
    logger.debug("Starting MMDB country download process")
    ...
    response = None
    if tmp_path.is_file():
        logger.debug("Temporary country MMDB file exists, checking if it's the latest version")
    ...
    if dl_mmdb:
        logger.debug("Checking job cache for existing country MMDB file")
        ...
        if isinstance(job_cache, dict):
            ...
            if response is None:
                logger.debug("No previous response, requesting country version check")
                response = request_mmdb()
        ...
        mmdb_url = f"https://download.db-ip.com/free/dbip-country-lite-{current_date.strftime('%Y-%m')}.mmdb.gz"
        logger.debug(f"Generated country MMDB URL: {mmdb_url}")
    ...
    if tmp_path.is_file():
        logger.debug(f"Validating country MMDB file: {tmp_path}")
        try:
            with open_database(tmp_path.as_posix()) as reader:
                logger.debug("Country MMDB file validation successful")
                pass
        except Exception as e:
            logger.error(f"Country MMDB file validation failed: {e}")
            raise

    # Move it to cache folder
    logger.info("Moving mmdb file to cache ...")
    logger.debug(f"Caching country file with hash: {new_hash}")
    cached, err = JOB.cache_file("country.mmdb", tmp_path, checksum=new_hash)
    if not cached:
        logger.error(f"Error while caching mmdb file : {err}")
        sys_exit(2)
    else:
        logger.debug("Country MMDB file successfully cached")

    # Success
    if dl_mmdb:
        logger.info(f"Downloaded new mmdb from {mmdb_url}")
        logger.debug("Country MMDB download and caching process completed successfully")
    ...
    status = 2
    logger.exception("Exception while running mmdb-country.py")
