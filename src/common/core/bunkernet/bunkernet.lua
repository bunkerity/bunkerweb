...
function bunkernet:initialize(ctx)
    ...
    debug_log(self.logger, "Initializing BunkerNet plugin")
    ...
    if get_phase() ~= "init" and self:is_needed() then
        ...
        if id then
            ...
            debug_log(self.logger, "BunkerNet ID found: " .. id)
        else
            debug_log(self.logger, "No BunkerNet ID found in datastore")
        end
    end
end
...
function bunkernet:is_needed()
    ...
    if self.is_loading then
        debug_log(self.logger, "BunkerNet not needed - loading phase")
        return false
    end
...
function bunkernet:init_worker()
    ...
    debug_log(self.logger, "Initializing BunkerNet worker")
...
function bunkernet:init()
    ...
    debug_log(self.logger, "Initializing BunkerNet database")
    ...
    debug_log(self.logger, "Retrieved instance ID: " .. id)
    ...
    debug_log(self.logger, "Loaded " .. tostring(i) .. " threat IPs")
...
function bunkernet:access()
    ...
    if is_whitelisted(self.ctx) then
        debug_log(self.logger, "Client is whitelisted: " .. 
            self.ctx.bw.remote_addr)
    ...
    debug_log(self.logger, "Checking IP against BunkerNet database: " .. 
        self.ctx.bw.remote_addr)
    ...
    debug_log(self.logger, "IP not found in BunkerNet database: " .. 
        self.ctx.bw.remote_addr)
...
function bunkernet:log(bypass_checks)
    ...
    if not reason then
        debug_log(self.logger, "IP is not blocked: " .. 
            self.ctx.bw.remote_addr)
    ...
    elseif err == "not found" then
        ...
        debug_log(self.logger, "Adding report to queue: " .. 
            encode(report))
    ...
    debug_log(self.logger, "IP already reported recently: " .. 
        self.ctx.bw.remote_addr)
...
function bunkernet:request(method, url, data)
    ...
    debug_log(self.logger, "Request data: " .. encode(all_data))
    ...
    debug_log(self.logger, "Response: " .. encode(ret))
...
function bunkernet:api()
    ...
    if match(self.ctx.bw.uri, "^/bunkernet/ping$") then
        debug_log(self.logger, "Handling ping request")
    ...
    elseif match(self.ctx.bw.uri, "^/bunkernet/reports$") then
        debug_log(self.logger, "Handling reports request")
        ...
        debug_log(self.logger, "Found " .. tostring(len) .. " reports")
