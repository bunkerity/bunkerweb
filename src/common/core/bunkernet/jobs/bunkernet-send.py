...
def get_cached_reports(job):
    ...
    if getenv("LOG_LEVEL") == "debug":
        LOGGER.debug("Getting cached reports from database")
    ...
    if not cached_data:
        ...
        if getenv("LOG_LEVEL") == "debug":
            LOGGER.debug("No cached data found, creating new cache structure")
    else:
        if getenv("LOG_LEVEL") == "debug":
            LOGGER.debug(
                f"Found cached data created: {cached_data.get('created')}"
            )
            LOGGER.debug(
                f"Cached reports count: {len(cached_data.get('reports', []))}"
            )
...
def get_instance_reports(api_caller):
    ...
    if getenv("LOG_LEVEL") == "debug":
        LOGGER.debug("Getting reports from all instances")
    ...
    if resp:
        ...
        if getenv("LOG_LEVEL") == "debug":
            LOGGER.debug("Failed to get reports from instances")
...
def merge_and_sort_reports(cached_data, instance_reports):
    ...
    if getenv("LOG_LEVEL") == "debug":
        LOGGER.debug(f"Total merged reports: {len(reports)}")
...
def should_force_send(cached_data):
    ...
    if getenv("LOG_LEVEL") == "debug":
        ...
        LOGGER.debug(f"Cache age: {cache_age}, force send: {force_send}")
...
def process_report_batches(reports):
    ...
    while force_send or remaining >= BATCH_SIZE:
        ...
        if getenv("LOG_LEVEL") == "debug":
            LOGGER.debug(f"Processing batch of {len(batch)} reports")
        ...
        if remaining >= BATCH_SIZE:
            if getenv("LOG_LEVEL") == "debug":
                LOGGER.debug("Sleeping 2 seconds before next batch...")
...
def cache_remaining_reports(job, reports, cached_data):
    ...
    if reports:
        ...
        if getenv("LOG_LEVEL") == "debug":
            LOGGER.debug(f"Caching {remaining_count} remaining reports")
        ...
        if getenv("LOG_LEVEL") == "debug":
            LOGGER.debug("No reports to cache, deleting cache file")
