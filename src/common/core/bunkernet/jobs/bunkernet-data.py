...
def get_instance_id_from_cache(job, bunkernet_path):
    ...
    if bunkernet_id:
        ...
        debug_log(LOGGER, f"Instance ID: {bunkernet_id.decode()}")
    else:
    ...
    debug_log(LOGGER, f"Instance ID file exists: {id_file_exists}")
...
def check_cache_freshness(job):
    ...
    debug_log(LOGGER, f"IP list cache is fresh: {is_cached}")
...
def download_threat_data():
    ...
    debug_log(LOGGER, f"Download response: ok={ok}, status={status}")
...
def validate_api_response(status, response_data):
    ...
    debug_log(LOGGER, f"Received {data_count} threat entries")
...
def save_threat_data(job, response_data):
    ...
    debug_log(LOGGER, "Saving threat data to cache")
    ...
    debug_log(LOGGER, f"New hash: {new_hash}")
    debug_log(LOGGER, f"Old hash: {old_hash}")
    ...
    debug_log(LOGGER, f"Cached {threat_count} threat entries")
