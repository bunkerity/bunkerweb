...
from jobs import get_file_in_db, set_file_in_db, del_file_in_db, send_cache_to_api
...
try:
    ...
    for instance in bunkernet_instances:
        instance_bunkernet_path = (
            bunkernet_path.joinpath(instance)
            if instance != "127.0.0.1"
            else bunkernet_path
        )
        instance_bunkernet_path.mkdir(parents=True, exist_ok=True)

        # Get ID from cache
        bunkernet_id = get_file_in_db("instance.id", instance, db)
        instance_id_path = instance_bunkernet_path.joinpath("instance.id")
        if bunkernet_id:
        ...
        if registered:
            cached, err = set_file_in_db(
                "instance.id", bunkernet_id.encode(), instance, db
            )
            if not cached:
        ...
        for i in range(0, 5):
            ...
            if not ok:
                logger.error(
                    f"Error while sending ping request to BunkerNet API : {data}"
                )
                retry = True
            elif status == 429:
                logger.warning(
                ...
                del_file_in_db("instance.id", instance, db)
                _exit(2)
        ...
        if instance != "127.0.0.1":
            ...
            sent, res = send_cache_to_api(instance_bunkernet_path, instance_api)
            if not sent:
