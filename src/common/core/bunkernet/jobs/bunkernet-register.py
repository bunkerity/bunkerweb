...
def get_cached_id(job):
    ...
    if getenv("LOG_LEVEL") == "debug":
        LOGGER.debug("Getting BunkerNet ID from cache")
    
    bunkernet_id = job.get_cache("instance.id")
    
    if getenv("LOG_LEVEL") == "debug":
        if bunkernet_id:
            LOGGER.debug(f"Found cached ID: {bunkernet_id.decode()}")
        else:
            LOGGER.debug("No cached ID found")
...
def register_instance():
    ...
    if getenv("LOG_LEVEL") == "debug":
        LOGGER.debug(f"Registration response: ok={ok}, status={status}")
    ...
    if getenv("LOG_LEVEL") == "debug":
        LOGGER.debug(f"Successfully registered with ID: {bunkernet_id}")
...
def cache_instance_id(job, bunkernet_id):
    ...
    if getenv("LOG_LEVEL") == "debug":
        LOGGER.debug(f"Caching instance ID: {bunkernet_id}")
