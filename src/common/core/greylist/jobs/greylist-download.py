...
try:
    ...
    for instance, config in configs.items():
        # Multisite case
        configs[instance]["GREYLIST_ACTIVATED"] = False
        if config.get("MULTISITE", "no") == "yes":
            for first_server in config.get("SERVER_NAME", "").split(" "):
                if (
                    config.get(
                        f"{first_server}_USE_GREYLIST",
                        config.get("USE_GREYLIST", "yes"),
                    )
                    == "yes"
                ):
                    greylist_instances.append(instance)
                    break
        # Singlesite case
        elif config.get("USE_GREYLIST", "yes") == "yes":
            greylist_instances.append(instance)

    if not greylist_instances:
    ...
    for instance in greylist_instances:
        # Our urls data
        urls = {
            "IP": [],
            "RDNS": [],
        ...
        for kind in urls:
            for url in configs[instance].get(f"GREYLIST_{kind}_URLS", "").split(" "):
        ...
        instance_greylist_path = greylist_path.joinpath(
            same_as or (instance if instance != "127.0.0.1" else "")
        )
        instance_greylist_path.mkdir(parents=True, exist_ok=True)
        ...
        if same_as:
            if instance != "127.0.0.1":
                sent, res = send_cache_to_api(instance_greylist_path, instance_api)
                if not sent:
                    logger.error(f"Error while sending greylist to API : {res}")
                    sys_exit(2)
                logger.info(res)
            continue
        ...
        for kind in kinds_fresh:
            if not is_cached_file(
                instance_greylist_path.joinpath(f"{kind}.list"),
                "hour",
                instance,
                db,
            ):
        ...
        for kind, urls_list in urls.items():
            ...
            for url in urls_list:
                try:
                    ...
                    instance_tmp_greylist_path = tmp_greylist_path.joinpath(
                        instance if instance != "127.0.0.1" else ""
                    )
                    instance_tmp_greylist_path.mkdir(parents=True, exist_ok=True)
                    instance_tmp_greylist_path.joinpath(f"{kind}.list").write_bytes(
                        content
                    )
                    ...
                    new_hash = file_hash(
                        instance_tmp_greylist_path.joinpath(f"{kind}.list")
                    )
                    old_hash = cache_hash(
                        instance_greylist_path.joinpath(f"{kind}.list"),
                        instance,
