...
class Database:
    ...
    def __init__(
        ...
        # Explicitly set sslcert/sslkey to empty strings so libpq skips client cert lookup.
        if sqlalchemy_string and "+psycopg" in sqlalchemy_string and "sslmode" in sqlalchemy_string:
            # libpq automatically looks for a client certificate at $HOME/.postgresql/postgresql.crt.
            # When running as a service user with HOME inherited from root (e.g. via setpriv),
            # this fails with "Permission denied". Setting sslcert/sslkey to a nonexistent path
            # tells libpq to skip client certificate authentication entirely.
