...
def upgrade() -> None:
    ...
    # Check if 'api' value already exists in enum before adding
    result = bind.execute(sa.text("SELECT EXISTS (SELECT 1 FROM pg_enum WHERE enumlabel = 'api' AND enumtypid = 'methods_enum'::regtype)"))
    ...
    if "bw_api_users" not in inspector.get_table_names():
        op.create_table(
            ...
            sa.Column(
                "method", postgresql.ENUM("api", "ui", "scheduler", "autoconf", "manual", "wizard", name="methods_enum", create_type=False), nullable=False
            ),
