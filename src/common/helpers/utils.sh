...
function check_instance_credential() {
	...
	log "$caller" "❌" "This instance was enrolled but its credential is gone ($INSTANCE_CREDENTIAL_FILE is missing or contains no usable credential) while $INSTANCE_ENROLLED_MARKER survived. The control plane still dials this instance with a credential it no longer holds. To recover: issue a new enrollment code (Instances page in the web UI, or POST /instances/<hostname>/enroll on the API) and pass it as INSTANCE_ENROLLMENT_CODE on the next start. To return an unrevoked instance to the shared API_TOKEN instead, first clear its control-plane credential using PATCH /instances/<hostname> with {\"credential\": \"\"}, then remove both $INSTANCE_CREDENTIAL_FILE and $INSTANCE_ENROLLED_MARKER and restart. For a revoked instance, re-enroll it. Refusing to start."
	return 1
}
