"""CrowdSec control through configured BunkerWeb instances, used by the API."""
...
class CrowdSecClient:
    ...
    def _instances(self) -> list[dict]:
        try:
            return self.db.get_instances(with_credential=True)
        except Exception:
    ...
    def _request(instance: dict, method: str, path: str, data: dict | None = None):
        try:
            ok, _message, status, payload = API.from_instance(instance, token=getenv("API_TOKEN") or None).request(method, path, data=data, timeout=(5, 30))
        except Exception:
    ...
    def _resolve(self, connection_id: str) -> tuple[dict, str]:
        ...
        for instance in self._instances():
            ...
            if isinstance(hostname, str) and sha256(hostname.encode()).hexdigest() == instance_key:
                if instance.get("status") == "down":
                    raise CrowdSecError("BunkerWeb instance is down")
    ...
    def connections(self) -> dict:
        ...
        for instance in self._instances():
            hostname = instance.get("hostname", "")
            if instance.get("status") == "down":
                errors[hostname] = "BunkerWeb instance is down"
                continue
            try:
