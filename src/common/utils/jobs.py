...
# `ATOMIC_TMP_SUFFIX` moved to `cache_restore` with the writer itself (dev 63a7f6a4d) and is
# re-exported here: `jobs` is what every job script and test imports.
...
class Job:
    ...
    def restore_cache(self, *, job_name: str = "", plugin_id: str = "", manual: bool = True) -> bool:
        ...
        if self.job_path.name == "modsecurity" and job_name == "download-crs-plugins":
            ...
            # The rendered ModSecurity configuration is the intersection of the manifest and the
            # directory, so half a pair renders a plugin set that never existed.
        ...
        for job_cache_file in job_cache_files:
            ...
            # Published by the archive's own transaction below, not on its own: a manifest written
            # here and a directory that then rolls back leave the two out of step.
            ...
            try:
                if job_cache_file["file_name"].endswith(".tgz"):
                    ...
                    if job_cache_file["job_name"] != job_name:
                        ...
                        continue
                    with LOCK:
                        # Staged, then renamed into place: an extraction that fails -- or a process
                        # killed mid-restore -- leaves the previous directory untouched instead of
                        # the empty post-rmtree state this code used to publish.
        ...
        with LOCK:
            ...
            if not job_cache_files and self.job_path.is_dir() and any(self.job_path.iterdir()):
                ...
                # `ret` gates the sweep: a row that failed to restore left the disk holding the
                # previous generation, and sweeping it against a cache set that row is missing
                # from would delete exactly the material the failure preserved.
