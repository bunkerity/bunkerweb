...
Release and staging runs share a queue because they use the same test infrastructure. Every required integration and platform startup check must succeed before the protected `release` environment can approve publication. That environment must keep required reviewers, and every reviewer must be one of the release owners (`TheophileDiot`, `fl0ppy-d1sk`); a release owner may approve their own run. The workflow checks this before building and before publishing and fails closed otherwise. A local unit or syntax check is not evidence that this release matrix has passed.
