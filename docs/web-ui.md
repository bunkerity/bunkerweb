...
=== "Docker"
    ...
    To forward the logs correctly to the `/var/log/bunkerweb` directory on the Docker integration, send NGINX access/error logs from the `bunkerweb` service via `ACCESS_LOG` / `ERROR_LOG`, and send service logs from other components via `LOG_TYPES` / `LOG_FILE_PATH` / `LOG_SYSLOG_ADDRESS` / `LOG_SYSLOG_TAG`. A syslog server writes everything to the shared volume. Here is an example of how to do this:
...
=== "Docker Autoconf"
    ...
    To forward the logs correctly to the `/var/log/bunkerweb` directory on the Autoconf integration, send NGINX access/error logs from the `bunkerweb` service via `ACCESS_LOG` / `ERROR_LOG`, and send service logs from other components via `LOG_TYPES` / `LOG_FILE_PATH` / `LOG_SYSLOG_ADDRESS` / `LOG_SYSLOG_TAG`. A syslog server writes everything to the shared volume. Here is an example of how to do this:
...
!!! tip "Web UI specific environment variables"
    ...
    - `UI_SSL_ENABLED`: set to `yes`/`true`/`on` to enable TLS directly on the UI service (defaults to `no`). Falls back to `SSL_ENABLED` if not set.
    - `UI_SSL_CERTFILE` and `UI_SSL_KEYFILE`: paths to the certificate and private key to use when `UI_SSL_ENABLED` is enabled. Fall back to `SSL_CERTFILE` and `SSL_KEYFILE` if not set.
    - `UI_SSL_CA_CERTS`: optional path to CA bundle if you need to load an intermediate/chain file. Falls back to `SSL_CA_CERTS` if not set.
...
=== "Docker"
    ...
    !!! info "Database backend"
        ...
        If you want another Database backend than MariaDB please refer to the docker-compose files in the [misc/integrations folder](https://github.com/bunkerity/bunkerweb/tree/v1.6.6/misc/integrations) of the repository.
...
=== "Kubernetes"
    ...
    !!! info "Database backend"
        ...
        If you want another Database backend than MariaDB please refer to the yaml files in the [misc/integrations folder](https://github.com/bunkerity/bunkerweb/tree/v1.6.6/misc/integrations) of the repository.
