...
Consulta la [carpeta de ejemplos](https://github.com/bunkerity/bunkerweb/tree/v1.6.15-rc3/examples) del repositorio para obtener ejemplos del mundo real.
...
=== "Interfaz de usuario web"
    ...
    * El **Modo sin formato** te permitirá introducir la configuración directamente como si editaras el archivo `variables.env`.
    ...
    === "Modo sin formato"
        ...
        Al principio de una línea de parámetro, pulse `#` para alternar su estado de borrador. Los borradores muestran un prefijo visual `#` atenuado; pulse **Retroceso** al principio de la línea para activarlos de nuevo. El prefijo es un metadato del editor y no modifica el valor del parámetro. Pulse **Guardar** para guardar el cambio. Mientras un parámetro sea borrador, se aplica su valor heredado o predeterminado. Los borradores se gestionan solo en modo RAW y se conservan al guardar en modo Simple o Avanzado. Este estado es independiente del borrador del servicio completo. Los parámetros bloqueados y los de identidad del servicio no pueden marcarse como borrador.
