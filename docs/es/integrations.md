...
| `TZ`                            | Zona horaria para logs del programador, jobs tipo cron, backups, marcas de tiempo y la programación `JOBS_DAILY_TIME` / `JOBS_WEEKLY_DAY`                                                                                                                                                    | Nombre en base TZ (ej. `UTC`, `Europe/Paris`) | unset (default de contenedor, suele ser UTC) |

`JOBS_DAILY_TIME` ejecuta los trabajos diarios a la hora local `HH:MM` (predeterminada `03:00`). `JOBS_WEEKLY_DAY` ejecuta los trabajos semanales a esa hora en un día de la semana (predeterminado `sunday`).
...
Esta plantilla está en [`misc/integrations/docker.yml`](https://github.com/bunkerity/bunkerweb/tree/v1.7.0-alpha/misc/integrations/docker.yml)
...
| Opción                  | Descripción                                                                                             |
| :---------------------- | :------------------------------------------------------------------------------------------------------ |
| `-v, --version VERSION` | Especifica la versión de BunkerWeb a instalar (p. ej., `1.7.0~alpha`).                                       |
...
=== "Debian Bookworm/Trixie"
    ...
    Ahora deberías poder instalar NGINX 1.30.5:
    ...
    Y finalmente instala BunkerWeb 1.7.0~alpha:
...
=== "Fedora"
    ...
    Fedora ya proporciona NGINX 1.30.5 que soportamos
...
Esta plantilla está en [`misc/integrations/autoconf.mariadb.yml`](https://github.com/bunkerity/bunkerweb/tree/v1.7.0-alpha/misc/integrations/autoconf.mariadb.yml)
...
!!! info "Dónde colocar los ajustes de Redis (configuración gestionada por el scheduler)"
    En Kubernetes, el **scheduler** lee los ajustes y genera la configuración que envía a las
    instancias de BunkerWeb; las instancias no leen los ajustes de Redis desde su propio entorno de
    pod. Con el chart de Helm, configure Redis bajo `settings.redis` —incluido Redis Sentinel mediante
    `settings.redis.redisSentinelHosts` y `settings.redis.redisSentinelMaster` (chart ≥ v1.0.21)— o en
    `scheduler.extraEnvs` para cualquier ajuste sin clave dedicada. Con Sentinel, `REDIS_HOST` **no**
    es necesario (el maestro se resuelve a través de los Sentinels). Definirlos solo en
    `bunkerweb.extraEnvs` no tiene ningún efecto.

    ```yaml
    redis:
      enabled: false        # clúster Redis/Sentinel externo
    settings:
      redis:
        useRedis: "yes"
        ...
        # redisPassword / redisSentinelPassword si su maestro/sentinels requieren auth
    ```
...
En lugar de usar el chart de Helm, también puedes usar las plantillas YAML dentro de la [carpeta misc/integrations](https://github.com/bunkerity/bunkerweb/tree/v1.7.0-alpha/misc/integrations) del repositorio de GitHub. Ten en cuenta que recomendamos encarecidamente usar el chart de Helm en su lugar.
...
!!! tip "Soporte PRO"
    **Si necesitas ayuda con la integración de Swarm**, por favor contáctanos en [contact@bunkerity.com](mailto:contact@bunkerity.com) o a través del [formulario de contacto](https://panel.bunkerweb.io/contact.php?utm_campaign=self&utm_source=doc).
...
Aquí está la plantilla de la pila, guardada en el repositorio en [`misc/integrations/swarm.mariadb.yml`](https://github.com/bunkerity/bunkerweb/tree/v1.7.0-alpha/misc/integrations/swarm.mariadb.yml) y verificada por la suite de pruebas como cualquier otra pila de referencia. Despliéguela con `docker stack deploy -c swarm.mariadb.yml bunkerweb`, **no** con `docker compose up`: un stack deploy ignora silenciosamente `container_name`, `depends_on`, `links`, `profiles` y la clave `restart:` de nivel superior, así que ninguna de ellas aparece abajo — el orden proviene del propio bucle de disponibilidad de cada componente más `deploy.restart_policy`. Las variantes de MySQL, PostgreSQL y Oracle están junto a ella (`swarm.mysql.yml`, `swarm.postgres.yml`, `swarm.oracle.yml`), y un `.ui.yml` hermano de cada una añade la interfaz web integrada.

Antes de desplegar, etiquete una vez el nodo que albergará los servicios con estado:
...
Las restricciones `node.labels.bw-state == true` anteriores anclan cada servicio con un volumen con nombre (`bw-db`, `bw-scheduler`, `bw-worker`, `bw-jobs-broker`) al nodo etiquetado — un volumen con nombre en Swarm es local a un nodo, así que reprogramar uno de estos servicios sin el anclaje lo arranca contra un estado vacío, silenciosamente; para el broker de jobs eso significa que cada job en cola se pierde sin que nada quede registrado en ningún sitio. Eso cambia failover por durabilidad: si el nodo anclado muere, esos servicios no vuelven a levantarse en otro lugar. La alternativa de alta disponibilidad es un driver de volumen compartido (NFS, CephFS, un driver de bloque en la nube) en los cuatro volúmenes, tras lo cual las restricciones pueden eliminarse — un prerrequisito de infraestructura que esta documentación no asume, así que una puerta de CI de un solo nodo no puede ejercitar ese camino y permanece sin verificar por pruebas automatizadas.

!!! danger "La pila publicada antes de la 1.7 no arranca en la 1.7"
    Una pila copiada de una versión anterior no tiene `bw-api`, ni `API_URL`/`API_TOKEN` ni `bw-worker`/`CELERY_BROKER_URL`: `bw-autoconf` entonces espera indefinidamente una API que nunca arranca, y ningún job en segundo plano (renovación de certificados, actualizaciones de CRS, sincronización de bloqueos, …) llega a ejecutarse jamás. Vuelva a desplegar desde el archivo de referencia anterior en lugar de parchear el antiguo.
