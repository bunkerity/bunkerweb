...
- `SERVICE_WORKER` - Defaults to `yes` when the Scheduler is enabled; set it explicitly to disable the local job worker or run it without the Scheduler
...
The image sets `CELERY_BROKER_URL=redis://127.0.0.1:6380/0`. With the local Worker enabled, this default starts a dedicated Redis broker inside the container. It binds only to loopback, uses a `256mb` memory limit with `noeviction`, and persists its queue through AOF files in `/data/broker`. Keep the `/data` volume across upgrades. When the memory limit is reached, new writes fail instead of evicting queued jobs or configuration-push leases.

The API, Scheduler, Worker, container healthcheck and commands started through `docker exec` use the same image-level URL. To use an external broker, explicitly set `CELERY_BROKER_URL` with its authentication and TLS parameters; the embedded broker then stays disabled. An empty URL is rejected when the Worker is enabled. With `SERVICE_WORKER=no`, the embedded broker is disabled too.

`USE_REDIS` and every `REDIS_*` setting configure only the WAF datastore. They neither redirect nor disable the dedicated broker, including when `REDIS_HOST` names an external Redis or `REDIS_SSL=yes`. See [switching an older AIO broker](upgrading.md#aio-broker-upgrade) before updating an existing 1.7 deployment that used the WAF Redis for jobs.
...
- To disable the WAF datastore, set `USE_REDIS=no`. The dedicated job broker remains enabled while the local Worker needs it.
...
!!! info "Database backend"
    Please note that our instructions assume you are using SQLite as the default database backend, as configured by the `DATABASE_URI` setting. However, other database backends are also supported. See the docker-compose files in the [misc/integrations folder](https://github.com/bunkerity/bunkerweb/tree/v1.7.0-alpha/misc/integrations) of the repository for more information.
...
| `TZ`                            | Time zone for scheduler logs, cron-like jobs, backups, timestamps, and `JOBS_DAILY_TIME` / `JOBS_WEEKLY_DAY` scheduling                                                                                                                                         | TZ database name (e.g., `UTC`, `Europe/Paris`) | unset (container default, usually UTC) |

`JOBS_DAILY_TIME` runs daily jobs at local `HH:MM` time (default `03:00`). `JOBS_WEEKLY_DAY` runs weekly jobs at that time on a day of the week (default `sunday`).
...
The Worker and the job broker are new in 1.7, and the API — which shipped in 1.6 but ran in no
reference stack — is now required: the Scheduler dispatches its jobs through the API onto the
broker instead of running them itself, and the Worker executes them. A stack without the three
comes up healthy and runs no background job at all.
...
- `bw-universe`: for BunkerWeb, the scheduler, the API, the worker and the job broker
...
Here is the stack boilerplate, kept in the repository at [`misc/integrations/docker.yml`](https://github.com/bunkerity/bunkerweb/tree/v1.7.0-alpha/misc/integrations/docker.yml) and swept by the test suite like every other reference stack. It uses SQLite; MariaDB, MySQL, PostgreSQL and Oracle variants live next to it (`docker.mariadb.yml` and siblings), and a `.ui.yml` sibling of each adds the embedded web UI.
...
| Option                  | Description                                                                                            |
| ----------------------- | ------------------------------------------------------------------------------------------------------ |
| `-v, --version VERSION` | Specifies the BunkerWeb version to install (e.g., `1.7.0~alpha`).                                           |
...
=== "Debian Bookworm/Trixie"
    ...
    And finally install BunkerWeb 1.7.0~alpha:
...
!!! info "Database backend"
    Please be aware that our instructions assume you are using MariaDB as the default database backend, as configured by the `DATABASE_URI` setting. However, we understand that you may prefer to utilize alternative backends for your Docker integration. If that is the case, rest assured that other database backends are still possible. See docker-compose files in the [misc/integrations folder](https://github.com/bunkerity/bunkerweb/tree/v1.7.0-alpha/misc/integrations) of the repository for more information.
...
Here is the stack boilerplate, kept in the repository at [`misc/integrations/autoconf.mariadb.yml`](https://github.com/bunkerity/bunkerweb/tree/v1.7.0-alpha/misc/integrations/autoconf.mariadb.yml) and swept by the test suite like every other reference stack. MySQL, PostgreSQL and Oracle variants live next to it (`autoconf.mysql.yml`, `autoconf.postgres.yml`, `autoconf.oracle.yml`), and a `.ui.yml` sibling of each adds the embedded web UI.
...
while the **autoconf, scheduler, API and worker** are defined as **single replicated [Deployment](https://kubernetes.io/docs/concepts/workloads/controllers/deployment/)**.

Since 1.7 the stack also carries an **API**, a **Worker** and a **job broker**: the scheduler no
longer executes jobs itself, it dispatches them through the API onto the broker and the worker
executes them. The reference manifests declare them as the `bunkerweb-api`, `bunkerweb-worker` and
`bunkerweb-jobs-broker` Deployments, with the matching `svc-bunkerweb-api` and
`svc-bunkerweb-jobs-broker` Services
...
A stack without them comes up healthy and runs no background job at all — no certificate renewal, no
blocklist refresh, no backup.

The job broker is a **separate** Redis/Valkey instance from the shared data store below, and the
reference manifest declares it as its own `bunkerweb-jobs-broker` Deployment for that reason: a
broker must be configured `noeviction` so queued jobs are never dropped, while a data store is
normally capped and left free to evict. `maxmemory-policy` is per-server and never per-database, so
a single server cannot carry both settings.
...
!!! info "Database backend"
    ...
    See docker-compose files in the [misc/integrations folder](https://github.com/bunkerity/bunkerweb/tree/v1.7.0-alpha/misc/integrations)
...
Instead of using the helm chart, you can also use the YAML boilerplates inside the [misc/integrations folder](https://github.com/bunkerity/bunkerweb/tree/v1.7.0-alpha/misc/integrations) of the GitHub repository. Please note that we highly recommend to use the helm chart instead.
...
!!! tip "PRO support"
    **If you need help with the Swarm integration**, please contact us at [contact@bunkerity.com](mailto:contact@bunkerity.com) or via the [contact form](https://panel.bunkerweb.io/contact.php?utm_campaign=self&utm_source=doc).
...
Here is the stack boilerplate, kept in the repository at [`misc/integrations/swarm.mariadb.yml`](https://github.com/bunkerity/bunkerweb/tree/v1.7.0-alpha/misc/integrations/swarm.mariadb.yml) and swept by the test suite like every other reference stack. Deploy it with `docker stack deploy -c swarm.mariadb.yml bunkerweb`, **not** `docker compose up`: a stack deploy silently ignores `container_name`, `depends_on`, `links`, `profiles` and the top-level `restart:` key, so none of those appear below — ordering comes from each component's own readiness loop plus `deploy.restart_policy`. MySQL, PostgreSQL and Oracle variants live next to it (`swarm.mysql.yml`, `swarm.postgres.yml`, `swarm.oracle.yml`), and a `.ui.yml` sibling of each adds the embedded web UI.

Before deploying, label the node that will hold the stateful services once:
...
The `node.labels.bw-state == true` constraints above pin every service with a named volume (`bw-db`, `bw-scheduler`, `bw-worker`, `bw-jobs-broker`) to the labelled node — a named volume in Swarm is local to a node, so rescheduling one of these services without the pin starts it against empty state, silently; for the jobs broker that means every queued job is lost with nothing logged anywhere. That trades away failover for durability: if the pinned node dies, those services do not come back elsewhere. The HA alternative is a shared volume driver (NFS, CephFS, a cloud block driver) on the four volumes, after which the constraints can be dropped — an infrastructure prerequisite this documentation does not assume, so a single-node CI gate cannot exercise that path and it stays unverified by automated tests.

!!! danger "The stack shipped before 1.7 does not boot on 1.7"
    A stack copied from an older release has no `bw-api`, no `API_URL`/`API_TOKEN` and no `bw-worker`/`CELERY_BROKER_URL`: `bw-autoconf` then waits forever for an API that never starts, and no background job (certificate renewal, CRS updates, bans sync, …) ever runs. Redeploy from the reference file above instead of patching the old one.
