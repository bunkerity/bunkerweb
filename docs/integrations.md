...
docker run -d \
  ...
  -p 443:8443/tcp \
  -p 443:8443/udp \
  bunkerity/bunkerweb-all-in-one:1.6.7-rc2
```
...
services:
  bunkerweb-aio:
    image: bunkerity/bunkerweb-all-in-one:1.6.7-rc2
...
```shell
docker pull bunkerity/bunkerweb:1.6.7-rc2
```
...
```shell
docker pull ghcr.io/bunkerity/bunkerweb:1.6.7-rc2
```
...
!!! info "Database backend"
    Please note that our instructions assume you are using SQLite as the default database backend, as configured by the `DATABASE_URI` setting. However, other database backends are also supported. See the docker-compose files in the [misc/integrations folder](https://github.com/bunkerity/bunkerweb/tree/v1.6.7-rc2/misc/integrations) of the repository for more information.
...
```shell
docker pull bunkerity/bunkerweb-scheduler:1.6.7-rc2
```
...
curl -fsSL -O https://github.com/bunkerity/bunkerweb/releases/download/v1.6.7-rc2/install-bunkerweb.sh
curl -fsSL -O https://github.com/bunkerity/bunkerweb/releases/download/v1.6.7-rc2/install-bunkerweb.sh.sha256
...
| Option                  | Description                                                           |
| ----------------------- | --------------------------------------------------------------------- |
| `-v, --version VERSION` | Specifies the BunkerWeb version to install (e.g., `1.6.7~rc2`).       |
...
=== "Debian Bookworm/Trixie"
    ...
    And finally install BunkerWeb 1.6.7~rc2:

    ```shell
    curl -s https://repo.bunkerweb.io/install/script.deb.sh | sudo bash && \
    sudo apt update && \
    sudo -E apt install -y --allow-downgrades bunkerweb=1.6.7~rc2
    ```
...
=== "Fedora"
    ...
    Fedora already provides NGINX that we support. Choose your Fedora version below to install the correct NGINX package:
    ...
    === "Fedora 42 / 43"
        ...
        !!! warning "NGINX 1.28.0 no longer available"
            Fedora 42 and 43 have removed NGINX 1.28.0 from their mirrors. You must install NGINX 1.28.1 instead.
    ...
    curl -s https://repo.bunkerweb.io/install/script.rpm.sh | sudo bash && \
  	sudo dnf makecache && \
  	sudo -E dnf install -y --allowerasing bunkerweb-1.6.7~rc2
    ```
...
!!! info "Database backend"
    Please be aware that our instructions assume you are using MariaDB as the default database backend, as configured by the `DATABASE_URI` setting. However, we understand that you may prefer to utilize alternative backends for your Docker integration. If that is the case, rest assured that other database backends are still possible. See docker-compose files in the [misc/integrations folder](https://github.com/bunkerity/bunkerweb/tree/v1.6.7-rc2/misc/integrations) of the repository for more information.
...
!!! info "Database backend"
    ...
    See docker-compose files in the [misc/integrations folder](https://github.com/bunkerity/bunkerweb/tree/v1.6.7-rc2/misc/integrations)
...
Instead of using the helm chart, you can also use the YAML boilerplates inside the [misc/integrations folder](https://github.com/bunkerity/bunkerweb/tree/v1.6.7-rc2/misc/integrations) of the GitHub repository. Please note that we highly recommend to use the helm chart instead.
