...
=== "API Settings"
    ...
    With `API_LISTEN_HTTPS=yes`, BunkerWeb creates a persistent, per-instance self-signed certificate under `/var/lib/bunkerweb` before the first configuration render. Internal-certificate private keys stay on the instance and never transit through a worker, the database or the API. Startup aborts without falling back to plaintext if the required certificate cannot be created. A valid pair is reused across restarts; a missing, corrupt or expiring (within 30 days) pair is replaced, changing its fingerprint. Clients that pin the certificate must then update the pin manually.
...
=== "Logging Settings"
    ...
    !!! info "Enriched context variables"
        ...
        | `$bw_city`         | City name, or empty (requires `GEOIP_CITY=yes`)                             |
        ...
        A request served by a server block that does not fill them (default server, internal API) logs `-`. GeoIP data comes from the databases managed by the [GeoIP](#geoip) plugin: when one is missing, `$bw_country` is `unknown`, `$bw_city` is empty, and traffic is served normally.
...
=== "Integration Settings"
    ...
    | `KEEP_CONFIG_ON_RESTART` | `yes`   | global    | No       | **Keep Config on Restart:** Keep the current configuration when the instance restarts, instead of falling back to the temporary startup configuration until the Scheduler sends a new one. The instance keeps enforcing while it waits. Set to `no` to regenerate the temporary configuration on every restart. |
...
The entrypoint also derives a stable `master_secret` for the challenge runtime the first time bot detection is enabled, and persists it under `/var/lib/bunkerweb` (the same volume as everything else the all-in-one image keeps). Without it, CrowdSec generates a new one on every restart and invalidates every outstanding challenge cookie; give the container a persistent volume for `/data` so the secret — and the rest of the instance's identity — survives a recreation.
...
| `LETS_ENCRYPT_DISABLE_PUBLIC_SUFFIXES`      | `yes`         | multisite | no       | **Disable Public Suffixes:** Refuse certificate requests for domains matching the [Public Suffix List](https://publicsuffix.org/) (recommended). Disable only if you knowingly need a certificate for a bare public suffix.                                                   |
...
The Let's Encrypt plugin performs DNS-01 challenges through [certbot-dns-multi](https://github.com/alexzorin/certbot-dns-multi), which embeds [lego](https://go-acme.github.io/lego/dns/)'s DNS providers — so **all of lego's 200+ DNS providers are supported**, not just the ones listed below. Set `LETS_ENCRYPT_DNS_PROVIDER` to a provider code and supply its credentials with the `LETS_ENCRYPT_DNS_CREDENTIAL_ITEM` setting.

Credentials may be given either with BunkerWeb's historical key names (shown in the table below, kept for backward compatibility) or with the provider's native lego environment-variable names (e.g. `CF_DNS_API_TOKEN` for Cloudflare). Any credential value can also be read from a file by appending `_FILE` to its lego variable name. For the complete provider list and the exact credential variables each one accepts, see the [lego DNS providers documentation](https://go-acme.github.io/lego/dns/).
...
!!! info "Using other providers and lego credential names"
    The table lists the providers BunkerWeb has historically documented; their legacy credential keys remain accepted. Any other lego provider works too — use its code as `LETS_ENCRYPT_DNS_PROVIDER` and its lego environment-variable names as `LETS_ENCRYPT_DNS_CREDENTIAL_ITEM` entries (see the [lego DNS providers documentation](https://go-acme.github.io/lego/dns/)). A few legacy names map to a different lego code — `google` → `gcloud`, `nsone` → `ns1`, `gandi` → `gandiv5`, `rfc2136` → `dnsupdate`, `domainoffensive` → `dode` — and both spellings are accepted.
...
| `MODSECURITY_SEC_REQUEST_BODY_LIMIT_ACTION` | `Reject`                        | multisite | no       | **Request Body Limit Action:** `Reject` answers `413` for a request over the body limit; `ProcessPartial` inspects only the portion within the limit and forwards the rest to the backend — useful for large uploads, but it reduces WAF coverage for non-file content types like JSON or form data. |
...
| Setting                   | Default | Context   | Multiple | Description                                                                                                       |
| ------------------------- | ------- | --------- | -------- | ----------------------------------------------------------------------------------------------------------------- |
| `REDIRECT_FROM`           | `/`     | multisite | yes      | **Path to redirect from:** The path that will be redirected. A value starting with `^` or ending with `$` is treated as a regex location. Optionally prefix with `~`, `~*`, `=` or `^~` followed by one space to set the nginx location modifier explicitly; no spaces, `;`, `{` or `}` are allowed elsewhere in the value. |
