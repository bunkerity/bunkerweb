...
Um die Registrierung bei der Central API und der Console zu deaktivieren, ohne dass Signale gesendet oder die Community-Blocklist abgerufen werden, setzen Sie `DISABLE_ONLINE_API` auf `true`. Dies stoppt keine Hub-Updates: Der Katalog für Collections und Parser wird unabhängig von dieser Einstellung weiterhin aus dem CrowdSec-Hub abgerufen.
...
```shell
docker pull bunkerity/bunkerweb:1.6.15-rc3
```
...
```shell
docker pull ghcr.io/bunkerity/bunkerweb:1.6.15-rc3
```
...
!!! info "KEEP_CONFIG_ON_RESTART"
    Anders als die obigen Einstellungen wird `KEEP_CONFIG_ON_RESTART` direkt vom Entrypoint des `bunkerweb`-Containers aus dessen Prozessumgebung gelesen, nicht vom Scheduler oder der Datenbank, und muss daher am `bunkerweb`-Container selbst gesetzt werden. Auf `yes` setzen, um die zuvor generierte Konfiguration bei einem Container-Neustart beizubehalten, anstatt die Lade-Konfiguration zu rendern. Standard `no`.
...
```shell
docker pull bunkerity/bunkerweb-scheduler:1.6.15-rc3
```
...
| Option                  | Beschreibung                                                                                 |
| ----------------------- | -------------------------------------------------------------------------------------------- |
| `-v, --version VERSION` | Gibt die zu installierende BunkerWeb-Version an (z. B. `1.6.15~rc3`).                        |
...
=== "Debian Bookworm/Trixie"
    ...
    Und installieren Sie schließlich BunkerWeb 1.6.15~rc3:

    ```shell
    curl -s https://repo.bunkerweb.io/install/script.deb.sh | sudo bash && \
    sudo apt update && \
    sudo -E apt install -y --allow-downgrades bunkerweb=1.6.15~rc3
    ```
...
=== "Fedora"
    ...
    !!! warning "Fedora 43 liefert NGINX 1.30.4"
        Fedora 43 stellt NGINX 1.30.5 noch nicht bereit, daher ist das BunkerWeb-Paket für Fedora 43 gegen NGINX 1.30.4 gebaut und erwartet `nginx-1.30.4`. Installieren Sie auf Fedora 43 diese Version; die Vorgabe wechselt auf 1.30.5, sobald Fedora 43 es veröffentlicht. Fedora 44 stellt 1.30.5 bereits bereit.
	...
	sudo dnf makecache && \
	sudo -E dnf install -y --allowerasing bunkerweb-1.6.15~rc3
    ```
...
Anstatt das Helm-Chart zu verwenden, können Sie auch die YAML-Vorlagen im Ordner [misc/integrations](https://github.com/bunkerity/bunkerweb/tree/v1.6.15-rc3/misc/integrations) des GitHub-Repositorys verwenden. Bitte beachten Sie, dass wir dringend empfehlen, stattdessen das Helm-Chart zu verwenden.
