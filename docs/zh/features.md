...
5.  **调整哈希成本（可选）：** 通过 `AUTH_BASIC_ROUNDS`（1000-999999999）在登录性能和密码哈希强度之间取得平衡。
...
| `AUTH_BASIC_ROUNDS`   | `656000`          | multisite | 是   | **哈希轮数：** 生成 htpasswd 文件时使用的 SHA-512 轮数（必须保持在 1000 至 999999999 之间）。值越低验证越快，但计算成本也越低。 |
