...
=== "API 设置"
    ...
    当 `API_LISTEN_HTTPS=yes` 时，BunkerWeb 会在首次渲染配置前，于 `/var/lib/bunkerweb` 为每个实例创建持久的自签名证书。内部证书私钥仅保留在实例本地，不会经由 worker、数据库或 API 传输。若无法创建所需证书，启动将中止，而不会降级为明文。有效的证书与密钥对会在重启时复用；缺失、损坏或剩余有效期不足 30 天的证书对会被替换，指纹也会随之变化。固定证书指纹的客户端随后必须手动更新固定值。
