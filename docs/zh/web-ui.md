...
服务的模式是 `standard` 或 `redirect_only` 二者之一。redirect-only 服务只做重定向：不带反向代理、不提供文件服务、无自定义 NGINX 片段、无模板，也不附加任何上游池或工作流——一旦 redirect-only 计费上线，它将不再占用 PRO 服务配额。除此之外它仍是普通服务：保留自己的页面、指标和在服务列表中的位置。

该声明是显式的，从不从重定向设置中推断，且通过服务设置页上的**服务模式**卡片完成，而非设置表单。如果该服务仍带有 redirect-only 服务不允许保留的内容，或缺少必须具备的内容——例如未设置 `REDIRECT_TO` 目标，或未将 `SERVE_FILES` 显式设为 `no`（否则默认即为 `yes`）——卡片会列出每条原因，操作在这些原因消除之前保持禁用。草稿状态的服务不显示此卡片——草稿本来就不计入配额——但通过 API 转换草稿的模式仍然有效。

服务列表会为每个符合条件的服务打上标记，无需逐个打开页面即可发现可节省的服务。
