...
=== "Web UI"
    ...
    *   **原始模式**将允许您直接输入配置，就像编辑 `variables.env` 文件一样。
    ...
    !!! info "固定的 `Default server` 条目"

        在 `MULTISITE=yes` 下，列表的第一行不是你的某个服务：它是保留的 **Default server**，用于响应不匹配任何已配置服务的请求。打开它以设置其使用的证书、TLS、请求头和错误页面。它不提供删除、克隆或草稿——参见[配置 Default Server](features.md#miscellaneous)。在 `MULTISITE=no` 下这一行不存在，列表从你自己的服务开始。
