...
有关真实世界的示例，请参阅仓库的 [examples 文件夹](https://github.com/bunkerity/bunkerweb/tree/v1.6.15-rc3/examples)。
...
=== "Web UI"
    ...
    *   **原始模式**将允许您直接输入配置，就像编辑 `variables.env` 文件一样。
    ...
    === "原始模式"
        ...
        在设置行的开头按 `#` 切换草稿状态。草稿显示带有淡化的可视 `#` 前缀；在行首按**退格键**可重新启用。此前缀是编辑器元数据，不会更改设置值。点击**保存**以保存更改。设置处于草稿状态时，使用继承值或默认值。草稿仅在 RAW 模式中管理，在简单或高级模式中保存时会保留。这与整个服务的草稿状态不同。锁定的设置和服务标识设置不能设为草稿。
