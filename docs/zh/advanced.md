...
GitHub 仓库的 [examples](https://github.com/bunkerity/bunkerweb/tree/v1.6.12-rc3/examples) 文件夹中提供了许多真实世界的用例示例。

我们还提供了许多样板文件，例如用于各种集成和数据库类型的 YAML 文件。这些都可以在 [misc/integrations](https://github.com/bunkerity/bunkerweb/tree/v1.6.12-rc3/misc/integrations) 文件夹中找到。
...
推荐值为 `67108864` 字节（64 MiB）。当数据库在容器中运行时，通过 `command:` 指令进行设置：
...
对于本地安装或外部数据库，请改为将该设置添加到服务器配置中（例如，在 `/etc/mysql/mariadb.conf.d/` 或 `/etc/my.cnf.d/` 下的专用文件中）：
...
完整示例可在 [`examples/mcp-stack/`](https://github.com/bunkerity/bunkerweb/tree/v1.6.12-rc3/examples/mcp-stack) 中找到：
