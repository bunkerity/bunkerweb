...
!!! warning "`LETS_ENCRYPT_DISABLE_PUBLIC_SUFFIXES` 现在按文档工作"

    1.6 对此设置的处理方向相反；如果运营者设置 `no` 以启用公共后缀列表检查，则必须设置为 `yes`（默认值）。在默认值 `yes` 下，检查现在会真正运行，因此不会再请求名称本身就是公共后缀的证书。
...
<!-- 锚点固定为 #rolling-back-to-1614：本页、四个翻译版本以及已发布文档都链接到它。
     标题不再写死版本号，以便在下一次 N-1 变更后仍然适用。 -->
### 回退到某个 1.6 版本 {#rolling-back-to-1614}
...
**原地降级**仅针对已验证为无损的版本/引擎组合提供，并且只在 **SQLite 和 PostgreSQL** 上可用。在
MariaDB 和 MySQL 上，1.7 的迁移无法反向重放——它会在中途中止，留下一个既不属于旧版本也不属于新版本的
schema——因此无论选择哪个目标，这些安装都必须从备份恢复。

已实测并提供两个回退目标：

| 目标 | 何时选用 |
| ---- | -------- |
| **1.6.15** | 紧邻 1.7.0 之前的版本，也是推荐的目标。路径更短：两步迁移而不是六步，出错的环节更少。 |
| **1.6.14** | 仍然经过实测并继续提供，适用于必须回到该版本的安装。多出四步迁移，其中一步在您于 1.6.15~rc3 或更新版本中创建过单项设置草稿时会**直接拒绝执行**：这些行在 1.6.14 中无法表示，而 1.7 也没有清除它们的手段。降级命令会检测到这一点，还原它自己的备份，并让您停留在原处——但能够成功的是回退到 1.6.15。 |

比 1.6.14 更旧的版本没有实测组合，此时命令只能提示您从备份恢复。
...
bwcli plugin backup downgrade 1.6.15            # 仅报告，不做任何更改
bwcli plugin backup downgrade 1.6.15 --execute  # 请求确认后再执行迁移
```

把三条命令中的 `1.6.15` 换成 `1.6.14` 即可以较旧的版本为目标；每一步的版本必须一致，因为步骤 2
的静默保持正是针对该目标建立的。
...
!!! danger "原地降级会破坏什么"
    ...
    `API_TOKEN` 完成注册。按用户的界面偏好设置会保留下来，但会失去原本的含义：1.6.x 会将它们全部当作
...
Encrypt 状态以及备份归档在两个版本之间保持不变。需要 1.7 版 API 的外部插件在 1.6.x 上无法使用，
必须移除或一并降级。
...
=== "Docker"
    ...
    === "使用安装脚本轻松升级"
        ...
        * **工作原理**：
            ...
            2. 升级决策
                * 已在运行相同版本：打印堆栈状态并退出。
                * 目标版本更旧：**拒绝执行**。安装脚本本身没有降级自动化功能，针对已迁移过数据库的旧版本包启动调度器会失败并陷入重启循环。请参见[回退到某个 1.6 版本](#rolling-back-to-1614)先恢复数据库，然后再用旧版本重新运行安装脚本。
...
=== "Linux"
    ...
    === "使用安装脚本轻松升级"
        ...
        *   **工作原理**：
            ...
            6.  执行升级
                *   安装新的 BunkerWeb 软件包版本。只有当目标 BunkerWeb 固定了不同的 NGINX 版本时才会重新安装 NGINX：1.6.13 和 1.6.14 固定 NGINX 1.30.4，而 1.7.0 要求 1.30.5，因此该升级确实会提升 NGINX 并重启它。从已经固定 1.30.5 的 1.6.15 升级时，NGINX 不会被改动。
            ...
            *   升级模式绝不会降级 NGINX，也绝不会安装目标 BunkerWeb 所固定版本之外的版本——NGINX 版本不匹配时模块无法加载。
