...
| 组件 | 作用 |
| ---- | ---- |
| **BunkerWeb** (`bunkerweb`) | 终止 TLS 并处理流量的 NGINX 实例。每个节点运行一个；它不自行管理配置，而是接收调度器的指令。 |
| **调度器** (`bw-scheduler`) | 管理设置和自定义配置，生成配置并推送到实例，决定每个任务何时运行。它通过 API 派发任务，不自行执行。 |
| **API** (`bw-api`) | 控制平面。Web UI 和自动化通过它读写数据，UI 从不直接访问数据库；`bwcli` 可通过 `BWCLI_API_URL` 连接它。调度器也通过 `POST /jobs/dispatch` 派发任务，因此 API 停机同样会停止所有后台任务。 |
| **Worker** (`bw-worker`) | 执行调度器派发的证书续期、封禁列表下载、备份等任务，并将结果写回数据库。 |
| **任务代理** (`bw-jobs-broker`) | 专用 Redis/Valkey 实例，在调度器与 Worker 之间传递任务。它独立于可选的 `USE_REDIS` 数据存储：任务代理必须禁止淘汰键，而数据存储通常设置内存上限并允许淘汰；`maxmemory-policy` 按服务器生效。All-In-One 镜像在容器回环端口 `6380` 上运行专用任务代理，与独立配置的 WAF Redis 并存。 |

Worker 和任务代理是 1.7 的新组件。API 在 1.6 已经提供，但当时是可选组件，1.6 参考堆栈均未运行 API；每个 1.7 堆栈都会运行它。可选 Web UI (`bw-ui`) 以及 [autoconf](integrations.md#docker-autoconf)、[Kubernetes](integrations.md#kubernetes) 和 [Swarm](integrations.md#swarm) 集成的 `bw-autoconf` 控制器与它们一同运行。
...
!!! info "更进一步"
    ...
    您将在文档的[高级用法](advanced.md)和仓库的 [examples](https://github.com/bunkerity/bunkerweb/tree/v1.7.0-alpha/examples) 目录中找到多站点模式的具体示例。
...
!!! info "更进一步"
    ...
    您将在文档的[高级用法](advanced.md#custom-configurations)和仓库的 [examples](https://github.com/bunkerity/bunkerweb/tree/v1.7.0-alpha/examples) 目录中找到自定义配置的具体示例。
...
- **调度任务（作业）**：调度器决定定期维护、计划更新、封禁列表和证书刷新等任务何时运行，并负责派发。从 1.7 起，它通过 **API** 将任务送入**任务代理**，由 **Worker** 执行并将结果写回共享数据库。缺少 API、Worker 或任务代理时，任务会被调度却始终不执行，也不一定出现明显错误；参见[后台任务始终不运行](troubleshooting.md#background-jobs)。
...
调度器始终作为独立进程运行。在容器集成中，它使用专用容器 (`bw-scheduler`)；[All-In-One 镜像](integrations.md#all-in-one-aio-image) 则在同一个容器内管理调度器、API 和 Worker。在 Linux 上，调度器使用独立的 systemd 单元 `bunkerweb-scheduler`，与 `bunkerweb`、`bunkerweb-api` 和 `bunkerweb-worker` 并存。
...
!!! tip "即用型社区模板"
    ...
    浏览 [BunkerWeb 模板仓库](https://github.com/bunkerity/bunkerweb-templates#installing-templates)并按照其安装指南操作。建议大多数用户使用 Web UI 方式；对于托管部署，文档中还介绍了插件目录结构。安装后，请通过简单模式或使用 `USE_TEMPLATE` 设置选择模板。
