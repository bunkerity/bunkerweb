...
=== "Linux"
    ...
    **Sur un nœud exécutant uniquement l'instance BunkerWeb**, installé avec `--worker` au sens de l'installeur (instance sans plan de contrôle), `disabled` est correct : il ne possède aucun job. Ne l'activez pas.
    ...
    !!! info "Les mises à niveau antérieures à 1.7.0 ont pu le laisser actif sur un nœud d'instance"
        Le paquet lit `WORKER_MODE`/`MANAGER_MODE`/`SERVICE_*` dans son environnement pour déterminer le rôle de l'hôte, et aucune mise à niveau ne les définit : ni `apt install bunkerweb=...`, ni le chemin de mise à niveau d'`install-bunkerweb.sh`, qui se termine avant de les exporter. Depuis la 1.7.0, le paquet retrouve lui-même la topologie : il lit le type d'installation enregistré lors de la dernière installation déclarée, et sur un hôte antérieur à ce marqueur, il se rabat sur « cet hôte exécute-t-il `bunkerweb-scheduler` ? ». Ni broker ni `bunkerweb-worker` ne sont activés sur un nœud qui répond non. Les mises à niveau antérieures prenaient un tel hôte pour un hôte autonome et **activaient et démarraient** `bunkerweb-worker` ainsi que le premier Redis trouvé — sur un nœud comme celui-ci, le `redis-server` de la distribution (ou `valkey`/`redis`), car une installation d'instance seule ne provisionne pas `bunkerweb-broker`. Si votre nœud est passé par l'une de ces mises à niveau, nettoyez-le une fois pour toutes avec les commandes ci-dessous.

        Aucun job ne lui est normalement envoyé : il utilise `redis://127.0.0.1:6379/0`, sans plan de contrôle pour alimenter cette file. **Exception :** si son `CELERY_BROKER_URL` pointe vers un broker **routable**, copié d'une installation `--broker-url` ou défini manuellement, ce Worker consomme réellement des jobs. Une URL de broker local provisionnée par l'installeur contient `127.0.0.1` : copiée ici, elle pointe sur ce nœud et provoque seulement des tentatives de connexion refusées. Pour supprimer ce Worker superflu, exécutez `systemctl disable --now bunkerweb-worker`. Une seule fois suffit à partir de 1.7.0 : la mise à niveau suivante constate que l'hôte n'exécute aucun scheduler et n'y touche plus.
