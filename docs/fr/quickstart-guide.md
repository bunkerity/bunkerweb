...
=== "Docker"
    ...
    services:
      bunkerweb:
        ...
        volumes:
          - bw-instance-data:/data # Nécessaire pour détecter un identifiant perdu après l'enrôlement de cette instance, voir Instances dans la documentation de l'UI web
...
=== "Swarm"
    ...
    Docker Swarm est de nouveau pris en charge dans la 1.7. La pile de référence — avec les services API, Worker et le courtier de jobs dont un déploiement Swarm a besoin, plus les exigences d'étiquetage de nœuds, `mode: global` et `mode: host` propres à Swarm — se trouve dans la [section d'intégration Swarm](integrations.md#swarm), pas ici : elle ne rentre pas dans le format court de cette page de démarrage rapide, et la dupliquer à deux endroits les laisserait diverger. Déployez-la avec `docker stack deploy`, pas `docker compose up`.
