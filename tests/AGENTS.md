...
- **The legacy harness is gone.** `tests/main.py`, the `*Test.py` classes, `tests/examples/*.json` and `legacy-requirements.{in,txt}` were deleted once the Swarm arm and Linux example mode landed. `staging-tests.yml` was its only caller; wave 11 deleted that file too and gave `staging.yml` its own `parse-tests-*` / `run-*-tests-*` jobs instead, the same shape `1.7-dev.yml`/`dev.yml` already use — see `tests/README.md`'s "The legacy harness (deleted)" for what that recovers (4 of the `staging:` block's 9 Linux distros — the 256-job matrix cap rules out all 9, see below) and what it permanently does not (real domains / real Let's Encrypt certs — no code path in this framework takes a runtime domain, see below). `staging-create-infra.yml` and `staging-delete-infra.yml` (the terraform+ansible provisioning that registered the `bw-docker`/`bw-autoconf`/`bw-linux` self-hosted-runner labels) were deleted alongside it — those specific labels are unreferenced anywhere now, not because the framework has no self-hosted runners at all: `integration-tests.yml`'s `setup` job still runs on `[self-hosted, fireactions]` for every arm, staging included. `tests/scripts/test.sh` is the entry point; the `core.py` / `api.py` / `ui.py` runners below it can be driven directly for a single spec, but nothing else orchestrates a run.
...
- **`linux-build.yml` no longer pushes a `<distro>-tests` image.** It used to (`ghcr.io/bunkerity/<distro>-tests:<release>`), gated behind a `TEST` input every pipeline passed `true`, but the sole consumer was the deleted staging arm — the Linux integration has always built its systemd runtime locally (`integration-tests.yml`, `docker build -f tests/linux/Dockerfile-<distro>`). Removed in wave 11, along with the now-dead `TEST` input, the ghcr login step, and `packages: write` on every caller's `build-packages` job (`1.7-dev.yml`, `dev.yml`, `ui.yml`, `staging.yml`, `release.yml`) — none of them write to a package registry through this workflow any more.
- **The Linux arm runs one distro — except from `staging.yml`, which runs 4.** `1.7-dev.yml`/`dev.yml`/`ui.yml` pass `--dev` and get `utils/integrations.yml`'s `dev:` block, `ubuntu/noble` alone; that is by design, they gate merges, not publishes. `staging.yml` (wave 11) omits `--dev` and reads the `staging:` block's 9 live distro rows, but then filters down to 4 (`ubuntu/noble`, `debian/bookworm`, `el/9`, `fedora/44`) in its `parse-tests-core` job — GitHub caps a single job's matrix at 256 entries, and 9 distros × the core spec count already exceeds that. See the comment there for the exact filter and the ceiling before it needs revisiting.
...
- **A stale app tree left in the host's `/var/www/html` by an old example-stack run collides with a fresh one.** Every harness container bind-mounts it (`tests/docker/docker-compose.bunkerweb.yml` and the scheduler/worker/all-in-one files alongside it) additively — `tests/scripts/utils.sh` already documents that the mount "never deletes what is already there" — so a leftover WordPress install, for instance, turns `/wp-admin` into a real directory nginx redirects with a `301` before the fresh container's app is ever reached (`AssertionError: /wp-admin: 301` on `admin_interface_probe`). Clearing it needs a privileged move/delete under `/var/www`, which is a host change to report, not something a lane does from inside a run.
