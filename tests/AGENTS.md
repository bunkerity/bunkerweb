...
- Selected examples have scenario descriptors in `tests/examples/<name>.json`, with `name` pointing back at the directory to deploy. They live here, not in the example folder, so that user-facing documentation carries no test assertions. Adding a framework-managed example scenario means adding a file here.
