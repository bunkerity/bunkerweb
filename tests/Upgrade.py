...
if distro == "ubuntu":
    ...
    sudo apt update && \
    sudo apt install -y nginx=1.30.5-1~jammy
    ...
    http://nginx.org/packages/ubuntu `lsb_release -cs` nginx" \
    | sudo tee /etc/apt/sources.list.d/nginx.list
    sudo apt update && sudo apt install -y nginx=1.30.5-1~jammy
    ...
    apt-get update && \
    apt-get install -y --no-install-recommends nginx=1.30.5-1~bookworm
    ...
    | sudo tee /etc/apt/sources.list.d/nginx.list
    sudo apt update && sudo apt install -y nginx=1.30.5-1~bookworm
    curl -s https://repo.bunkerweb.io/install/script.deb.sh | sudo bash && \
    sudo apt update && \
    sudo apt install -y bunkerweb=1.4.5
    """
    ...
    wget https://nginx.org/packages/rhel/9/x86_64/RPMS/nginx-1.30.5-1.el9.ngx.x86_64.rpm
    dnf install nginx-1.30.5-1.el9.ngx.x86_64.rpm -y
    dnf install /data/bunkerweb.rpm -y
    """
        if distro.endswith("9")
        else """
    dnf install yum-utils wget sudo -y
    wget https://nginx.org/packages/rhel/8/x86_64/RPMS/nginx-1.30.5-1.el8.ngx.x86_64.rpm
    dnf install nginx-1.30.5-1.el8.ngx.x86_64.rpm -y
    dnf install /data/bunkerweb.rpm -y
    """
    )
