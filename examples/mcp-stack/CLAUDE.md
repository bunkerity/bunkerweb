...
This example deploys BunkerWeb (v1.6.12+) with its MCP server, giving AI assistants (Claude Code, Claude Desktop, etc.) full management capabilities over the WAF through the [Model Context Protocol](https://modelcontextprotocol.io/). It requires the external API and the Celery worker architecture introduced in 1.6.12.
...
This brings up BunkerWeb, the scheduler, API, MCP server, database, Redis, and a Celery **worker**. The worker is required: the scheduler dispatches jobs through the API to the worker queue, so without it `jobs_run` requests are accepted but never execute.
...
| **Cache**         | Data files generated by jobs (IP lists, GeoIP DBs, CRS rules), stored per-plugin under `/var/cache/bunkerweb/<plugin_id>/`. Reports, bans, and stats are persisted in Redis. |
