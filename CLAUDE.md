...
- **Worker** (`src/worker/`): Celery-based distributed job executor. `app.py` configures the Celery app with a Redis/Valkey broker and two queues, `default` and `heavy`; the heavy-job set and the routing function live in `src/common/utils/job_queues.py` (`HEAVY_JOBS`, `queue_for`) and are re-exported from `app.py`. `tasks.py` defines the `execute_job` task; `executor.py` dynamically loads plugin jobs from `src/common/core/*/jobs/`. Separate default and heavy worker processes run independently of the Scheduler and write results to the shared DB. `CELERY_BROKER_URL` defaults to `redis://127.0.0.1:6379/0` — a literal IPv4, deliberately, because `localhost` resolution is ambiguous on dual-stack hosts. The AIO image overrides it with `redis://127.0.0.1:6380/0` for its dedicated, non-evicting broker; its WAF Redis remains independent.
...
- **Default server** — the block answering a request matching no configured service (unknown SNI, a raw IP, an unknown `Host`) is a permanent, reserved `default-server` pseudo-service row in `bw_services`, not something rendered from global settings alone — but only when `MULTISITE=yes`; with it off the row is never seeded and the default server renders exactly as it did pre-1.7. `src/common/utils/default_server.py` holds its id, its curated plugin subset and its stream-port election; `Templator` renders it through three phase runners restricted to that subset instead of the per-service ones; `db_methods/services.py` seeds the row idempotently (data, not schema — it reuses the existing `wizard` method value); every mutation path (API, UI, config-save reconciliation) refuses to delete, rename, draft or bill it.
- **Instance enrollment** — an instance can mint its own credential from a single-use, time-limited code instead of sharing the global `API_TOKEN`. `POST /instances/{hostname}/enroll` (guarded) issues the code; `POST /instances/enroll` (deliberately the only unguarded route in `api/app/routers/instances.py` — it is what the booting instance itself calls, before it has any credential) redeems it. The DB lifecycle (issue, redeem, rotate, revoke) is `db_methods/instances.py`; the instance side persists the credential to `/var/lib/bunkerweb/instance-credential.json` and refuses to start if it goes missing after enrolling. An unenrolled row falls back to the global `API_TOKEN`; an enrolled one never does; a revoked one is refused at the dial (`common/api/API.py`) rather than falling back.
...
The legacy harness (`tests/main.py`, the `*Test.py` classes, `tests/examples/`) was deleted once the Swarm arm and Linux example mode closed its last two gaps. `staging-tests.yml`, its only caller, was deleted too; `staging.yml` gained its own `parse-tests-*`/`run-*-tests-*` jobs in its place.
